...
\label{tab:6:table6}
 ...
 $ \lambdabar_e ~(p_{hol}/p_t)^2~ e_3 ^{\sqrt a/2}$ & 13.81198 & Shows the arithmetical liaison between $p_t^2$ and $p_{hol}^2 = 4a^3/3$ ~~: $(p_{hol}/p_t)^2 \approx 137(1-136/a)^{1/2}$ (6 ppm)    \\
...
The mean nucleotide mass is $ (o_1 + o_2)/6 \approx 306.4032199$, close to $\pi^5 \approx 306.02$, the sixth part of the Lenz-Wyler proton-electron mass ratio \cite{Wyler} $6\pi^5$, which shows a geometric property: it is the product of the area by the volume of a cube of side $\pi$. The mean DNA bi-codon mass  $m_{bc}^{(0)}/m_H = (6/4)(o_1 + o_2) \approx 1838.418122$ connects very precisely (200 ppb, 150 ppb, 100 ppb, 50 ppb, 11 ppm, 1 ppm) with the following main parameters, 329 being the above Guanine nucleon number) and the central term $f(16)= e^{16}$ of the Topological Axis:

\begin{equation}
 \left\{
    \begin{array}{ll}
    329 \approx (\sqrt Z / 2g_2)(a/137)p_W/\beta p_t \\
        m_{bc}^{(0)}/ m_H  \approx H(n_t/p_t)^{1/2} \approx (\tau / 3\mu)(329~G_u^{(0)}d_e)^{1/2}\\
