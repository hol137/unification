...
Now, a cosmology founded on \textit{integral} equations must take into account the well-known gravitational potential energy $ E_{pot} -(3/5)GM^2/R$ of an homogeneous ball of mass $M$ and radius $R$. For the critical radius $R = 2GM/c^2$, this is the energy $ E_{pot} = -(3/10)Mc^2$, which is exactly the opposite of the \textit{non-relativist} kinetic energy $E_{kin}$ of the receeding galaxies in the steady-state model, \cite {Sanhez2} which is so characterised by the classical integral equation: 

\begin{equation}\label{Eq2}
E_{kin} = - E_{pot} = (3/10) Mc^2 
 \end{equation}

Thus, the matter + dark matter 0.3 density is directely explained. 

\textit {So, the deepest mystery of standard cosmology is the clearest evidence for its opponent simplest model, the non-relativist permanet Newtoniazn model: the standard cosmology is unduly complicated.} 
...
This means that the Restricted Relativity was unduly applied to cosmology. Indeed, it is a \textit{local} theory, based on the concept of an inertial referential, which cannot be defined by theory. However it is observationally evident: the Cosmic Microwave Background (CMB). In liaison with the star on-going production of Helium, this thermal bath was predicted by the steady-state model, with the correct temperature (contrary to the erroneous prediction of Gamow) \cite{Hoyle2}. Even more, the precise thermal spectrum was contradictory with the initial standard model, which was obliged to introduce the monstruous inflation step, with as many problems as it resolves \cite{Sternhardt}. Moreover, one of the three conditions of Sakharov for the elimination of antimatter during the Big Bang is precisely the rupture of thermal equilibrium. 
...
So \textit{the prediction of the concurrent model has been simply forgotten.}
...
Even Eddington did not emphatize that his above formula induces a $R$ value independent of $c$. This is due to the catastrophic use of the units where $c = 1$, where Time is assimilated to Space. In his basic text founding Restricted Relativity, Poincare \cite{Poincare2} insisted on the fact that the 4D space-time concept must not be systematised.  
...
The general tendancy of the \textit {formalist mainstream} has been to neglect such correlations, invoking either the hasard or a multiplicity of Universes (Multiverse hypothesis). However, the above Eddington Large Number Correlations are now extended to a Bott octonion sequence, by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory \cite{Sanchez2}. The latter was \textit {unduly} discarded because it involves a tachyon. Quite the contrary this is considered an advantage, to interpret the above non-Doppler quasar oscillation.
