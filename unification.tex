...
The associated nucleotide to the Guanine is the Cytosine, which is clearly tied with the topological function f(10), giving rise to a formula in the Hubble Table. The other couple (AT), as shown before (Eq. 19), is associated with the Fermi constant.


\subsection{The "free parameters" and the Euler ideonal numbers} 
The last known Euler ideonal number $s_{65} = 1848 = 43^2 -1 \approx (4\pi)^2\sqrt {137}$ is related to the dimension 4:equivalent relation for dimension 4, implying $\textbf{N}_4 = 4 K_4$ shows up a relation between the brute proton-electron mass ratio $1836$ with the Euler maximal suitable number $s_{65} = 1848 = 43^2 -1 \approx (4\pi)^2\sqrt {137}$:
\begin{equation}
