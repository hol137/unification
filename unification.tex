...
{\color{red} Contrary to old tradition, the present science is characterised by a mutiplicity of separated disciplines. In particular, the Number Theory does not play any role in Physics. Even the String Theory does not connect with Reality. Worse, the two pillars of Physics does not agree on the real status of space-time, and the standard model is incapable to define the values of about 30 "free parameters". Adopting the Optimal Computation Hypothesis, this article connects the Number Theory with a reunified Physics, implying chemical, biologic and cristallographic parameters untill 0.1 ppm precision. The debunking of the arithmetical origin of the main free parametersThis involves the the Wieferich and Mirimanoff prime couples, i.e. the base 2 and base 3 particular cases of the Fermat Petit Theorem. Also the Euler maximal suitable number plays a central role, showing the connection with the genaralised Riemann conjecture. In particular, this leads to the fine-structure constant within its 0.15 ppb indetermination. This confirms the Eddington's Fundamental Theory, in particular the non-standard symmetry proton-tau. The running number of the string dimension series of the Topological Axis is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, and leads to stringent predictions.  } 
...
% $ Rp_t (6\d_e P^{\sqrt a -4}/\pi)^{1/3}$ & 9.07574 & from the cosmic atomic mass $M_c/m_H \approx P^{\sqrt a}$ \\
 ...
 $ 2 (sin_Z \theta_{eff}){-4}\lambdabar_{e} (4a_s)^{64}$ & 9.07777 & empiric, implying a relation between $a$ and $a_s$ \\
