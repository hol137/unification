...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Golden ratio ~~~~$\gamma/\pi = \Gamma/a$ \cite{Atiyah} $\approx ln\Phi / \Phi^2 \approx ln(\xi(3)) $~~~~~~$\Phi^{\pi/\gamma} \approx e^{\Phi^2}\approx a/10$ & $\Phi$  & 1.61803398874990 & $\xi(3) \approx 1.20205691$ \\ 
    Wien parameter $W_5 = hc/k_BT \lambda_{Wien} = 5(1-e^{-W_5})$ &  $W_5$  & 4.96514245  & $W_3\approx~2.821439372$ \\
...
Note that the electric charge is tied to the Mirimanoff prime 11 \cite{Mirimanoff}, showing the connection supersymetry-supergravity 10 = 11 - 1. The small difference between $g_0$ and $g_2$ induces formula in the Hubble and Cosmos Tables. So, the gauge couplings are connected with the  Wien parameter  $W_5$. The dimension 11 in the Topological Axis is tied to the Kaon. Indeed, the "strange" mesons $K_0 \approx 973.784$ and $K_+ \approx 966.119$ appears in liaison with Eddington's parameters and the Wien factor $W_5/5 = 1-e^{-W_5}$. \textit{This means there is a symmetry between radiation and matter, totally unexpected in the present standard theory. This relation (Table 2) has the same form than that of Eddington: $\sqrt{M/m_e} = R/2\lambdabar_H$, equivalent to Eq. (5)}, becoming very precise (0.1 ppm) in the above last relation.
