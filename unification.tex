...
\node [anchor = text=black] at (-15.,34.75) {\small{Les longueurs caractéristiques entourent la loi: $f_b = b^{2^{d/4}}$~ for~ b = e}};
...
\node [cross,anchor = east,color=black!40!green] at (2.15,3.15){$RAM_{137}\approx a \sqrt{m_em_P}/m_H ~ (8 ppm)\approx f(18) (R_e/R)^2$};
...
\node [anchor = east,color=red] at (17.25,17.350){$f(25)\approx 2e^{24\pi}\approx O_B~ (W/F) \approx \sqrt {O_M}~ (8W \approx 7Z \approx 5aH) $};
...
\node [anchor = east ,color=red] at (17.3,19.0){$f(26)\approx~ $ NOMBRE PREMIER DE LUCAS $(2^{2^{2^{2^2-1}-1}-1}-1)\times 12$~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~};
...
\node [anchor = east,color=black!40!green] at (27.95,24.250){$(a^{1/3}/2)f(1/(a-137)) \approx O_M \approx e^{137e/3} \approx f(30)^{2/3} \Rightarrow a \approx 137.0359999099=$ mesuré (0.15 ppb)~~~~~~~~~~~~};
\node [anchor = east,color=black!40!green] at (25.95,22.50){$ f_3(1/(a-137)) \approx \sqrt a sin \theta (H/p)e^a \Rightarrow a \approx 137.0359999099$: idem (0.03 ppb)~~~~~~~~~~~~};
...
\node [anchor = east,color=red] at (22.,21.0){$RAM_{\sqrt(pn_t)}\approx (O_MO_B)^{1/2}\approx P^2/4\pi \approx f(a^{2/3}) \approx f(2p/a)  \approx a^{2a^2/n_t} \approx n_t^{n_t/a} \approx e^{32\pi} \approx \Phi^{210} \approx \phi^j $};
...
\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les rapports de longueur $Y /\lambdabar_e$ entourent la loi $e^{2^{d/4}}$, qui réunit 8 relations holographiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui s'avère fonction de la série du Tableau Périodique, $d = 4k + 2$, de $k = 0$ à $k = 7$, séquence d'octonions de Bott. \textbf{L'Univers observable est permanent, boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Faible (k=3), Fort (Gluons k=1)}. La dimension 26 des cordes bosoniques donne 12 fois le demi-rayon de Hubble $R/2$, voisin du Nombre Premier de Lucas, tandis que le Topon correspond à $f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e $. D'où $R \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse moyenne qui apparaît dans la Super-Période $ t_{sp} =\hbar^4/G_F^{5/2}\rho_c^{3/2} = \hbar^3/G^2m^5 \approx 5.4829 \times 10^{57} ~s$, avec la constante de Fermi $G_F \approx 1.435850991 \times 10^{-62} kg m^5 s^{-2}$ et la densité critique $\rho_c = 3c^2/8\pi R^2 = M/(4\pi R^3/3)$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = 2\hbar^2/Gm_N^3$. La relation $M/m_n \approx (10/3)N_{Ed}$ conduit à $m_n/m_e \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). Le nombre $f(18)/ \pi$ est voisin du Nombre Premier d'Euler et du rapport thermique, relié aux points triples de l'Hydrogène, de l'Oxygène et de l'Eau. \textbf{Le cardinal du Monstre $O_M$ définit la constante électrique $a \approx 137.035999099 $ conforme à la mesure (0.15 ppb), avec confirmation à 0.03 ppb, où $\sqrt a sin\theta = 2H^0 \sqrt{2+ln \pi} /W$}, et vérifie $H^{16} \approx O_M \times \phi_0^8$, avec $\phi_0^8 \approx 48$, montrant que $H = m_H/m_e$ est base de calcul. La période de Kotov $t_K $ de l'oscillation non-Doppler tachyonique des quasars est liée au Tempon par $t_C/t_K \approx O_M/\sqrt2 \approx e^a/F$. Le rayon holographique du proton $p_{hol}^2 = 4a^3/3 $ intervient (1 ppm). La dimension 12 confirme la symétrie d' Eddington Proton-Tau, ignorée par le Modèle Standard. \textbf{La constante $e^{\pi \sqrt{163}}$ et la fonction de partition de Ramanujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ pour n = 137 et $\sqrt{pn_t}$ jouent un rôle central}, illustrant la relation géo-combinatoire $p^{p^2}\approx (a^2)^{a^3}$ et la relation: $ f(a^{2/3})\approx (4\pi/3) \sqrt{f(26)f(27)}$. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes de la famille du Montre est lié à $a^a$ et au volume du Cosmos. $f(14) = f(10)^2$ implique $g = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 $, où $3570 = (2+3+5+7)(2\times 3\times 5\times 7) $ confirmant, à $10^{-8}$ près, le boson de Higgs $495^2$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/G;~ P=m_P/m_e;~p = m_p/m_e \approx 1836.152673;~H = m_H/m_e \approx 1837.152646~;~n_t= m_n/m_e \approx 1838.683661;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652;~ j= 8\pi^2/ln2 \approx 113.9;~ \Phi = (1+\sqrt5)/2$ (Nombre d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nombre d'Argent). \textbf{La masse du Bicodon d'ADN apparaît comme centrale}, liée au Monstre $ O_M \approx (\Phi_0 H^2)^8$. En particulier $\sqrt{Rl_P}\approx hc/k_BT(40^0C)$. Le Neuron $ t_0 = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ vérifie $Ct_0 / \lambdabar_e \approx 5^2 f(30)$, confirmant la super-vitesse $C = c(P/a^2)^3pH)(17 ppm)$, et $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (10 ppm). } }}
