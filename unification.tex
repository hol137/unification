...
\textit {So, the deepest mystery of standard cosmology is the clearest evidence for its opponent simplest model, the non-relativist steady-state model: the standard cosmology is unduly complicated.} 

Recall that the Newtonian Cosmology is sufficient to obtain the principal results from General Relativity, as shown by Bondi \cite {Bondi}, in particular the above critical formula $R = 2GM/c^2$, which states simply that the liberation speed at the surface is the classical non-relativist speed $(2GM/R)^{1/2}$, identified to $c$. So one may consider that receeding galaxies reach the speed $c$ and exceed it when they disappear at the !hubble horizon.
