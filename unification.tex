...
\node [anchor = text=black] at (-23.,34.75) {\small{Les grandeurs, rapportées à l'électron, entourent la loi, pour~ b = e: $f_b = b^{2^{d/4}}$~~~ (1 ppm = $10^{-6})$ }};
...
\node [anchor = east,color=red] at (-12,-18.0) {Mixage $ S \approx 1/0.231211149~ (0.01 ppm) $};
...
\node [anchor = east,color= red] at (-19.5,-15.5) {~~~~~~$ \Phi \approx \frac {a^3  sin \theta}{1836^2} $};
...
$\Delta_{Ed} = \sqrt{136^2-40} \approx 2a/\pi g_2$
\node [anchor = east,color=blue] at (22.75,-6){\LARGE{MACRO-PHYSIQUE}};

\node [anchor = east,color=red] at (30,-6){$\Delta_{Ed} = \sqrt{136^2-40} \approx 2a/\pi g_2$};
...
\node [anchor = north,color=red] at (-1, 35) {$\hbar,G, u  m_{bicodon}, m_{photon}, m_{graviton} \Rightarrow  R_{Cosmos} = R_e^2/2l_P  \approx \lambdabar_e \frac {6^{128}}{1+1/\sqrt 2} \approx l_P (6^{p_G/ \sqrt a} \approx \Phi^{17a^2/ (4\times 137)} \approx 3570^{137/4}) $};
