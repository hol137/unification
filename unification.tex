...
\draw[color=blue] (A) -- (Y);
%\node [anchor = text=blue] at (-8.,35.75) {\LARGE{AXE COSMIQUE} };{p}
...
\node [anchor = east,color=red] at (13,14.5) at (18,18){$ 3^{3^3-1} \approx (2^{2^2-1})^{\sqrt {a_{00}}/2} \approx a^ {(\frac {137}{a})^2\sqrt {a_s}/2} \approx {a_s}^{a^{11}/P} \Rightarrow a^a \approx \sqrt {a_s} ^  {\sqrt {pH/a_s}} $};

%\node [anchor = east,color=red] at (13,14.5){$\hbar,G,G_F \rightarrow 52,255 Hz \rightarrow $ Lab (La = 442,9 Hz)  };

\node [anchor = east,color=red] at (18,18)] at (13,14.5){$\hbar,G,G_F \rightarrow 52,255 Hz \rightarrow $ Lab (La = 442,9 Hz)  };
