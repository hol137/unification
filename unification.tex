...
Such a pertinence of the Eddington Number means that Number Theory enters Physics: this is the main goal of the present article to confirm this. According to Russel \textit{"The most surprising in the modern science is its come back to Pythagorism"}\cite{Alcina}. Indeed, every formalist followed Dirac when he interpreted the Cosmic correlation as age-varying, with a gravitation constant $G$ varying with time.

\textit{It was forgotten that Poincare clearly specified that Physics is not possible with varying constants \cite{Poincare2}.}

The String Theory uses only partially the Number Theory. With its continuum space, its compactification process results in an enormous multiplicity of solutions $(10)^{500}$ \cite{Schwarz}. One goal of this article is, by replacing the continuum by a Toponic quantinuum, to connect more tightly the two domains. In particular, the jonction is made with the celular automaton physics \cite{tHooft}.     
...
Combined R23W: $v_d = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & $u_{8}$    & $ \underline {1831} $ & from 30D to 12D \\ 
 ...
 R23W for $d$ = 8:  Eddington's formula: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & \cite{Eddington} \\   
  ...
  Combined R23W for d = 1 & $u_{2\times (1+3)}$    & $ \underline {137} $ & from 12D to 1D \\  
    ...
    Sum $4^2 + 11^2$ implying usual 4D  and supergravity 11D    & $a_{00}$    & $\underline {137}$ & $v_1 = 4$; $v_2 = 11$ \\
...
%\label{tab:4:table4}
  \hskip-0.0cm\begin{tabular}{llllllllllllll}
    ...
    $K_d$ (positive $PSO_{Cr}$)  & 1 & 5 & 5 & 19 & 19 & 59 & 59 & 165 & \underline{165} & 419 & 419 & 1001& 1001 \\
     ...
     $q_d=\binom{3+d}{4}$ (pentapope nbrs)  & 1 & 5 & 15 & 35 & 70 & 126 & 210 & 330 & \underline {495} & 715 & 1001 & 1365 & \underline{1820}\\
