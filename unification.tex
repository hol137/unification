...
\node [anchor = west,color=blue] at (-12.,-12.) {$3f(10)  \approx 2\pi r_H$ (Rayon de l'atome d'Hydrogène)/$\lambdabar_e \approx (\frac{2}{u})^{16} \approx (3570)^{16}/O_M$ };
...
\node [anchor = west,color= blue] at (-21,-23.7) {$ 7\times (\underline {s}+1) + 6\times (\underline {p}+1) + 4\times (\underline {d}+1) + 2\times (\underline {f}+1) = 107 + 30 = 137 \approx a = a cos^2 \theta + a sin^2 \theta$ };
...
\node [anchor = east,color= red] at (30,-26) {~~~~~~$ \Phi sin \theta \approx \frac {a^3  }{1836^2} $};
...
%\node [cross,anchor = east,color=red] at (-20,4.75){NOMBRE PREMIER D'EULER $(2^{2^{2^2+1}-1}-1) \times \pi ~~ $};
...
%\node [anchor = east,color=red] at (28,18){$ \frac {R}{2 \sqrt{\lambdabar_p \lambdabar_H}}  = \sqrt{\frac {H}{p} (\frac {M}{m_e}  = \frac {d}{\lambdabar_e})} = \frac {\hbar c}{Gm_em_p}  $};
...
%\node [anchor = east,color=blue] at (36.7,39){$ a^a \approx (e ^ {1/e})^{p_G (n_t/p)^  {1/2} } \approx (\sqrt {a_s} ^  {1/ \sqrt {a_s}})^{\sqrt {pH}} \approx ((sin \theta) ^  {-sin \theta})^{(a/\pi)^2} \approx (q ^  {-q})^{2(a sin^2 \theta)^2} \approx (\pi ^ {1/\pi})^{p_{hol}} \approx (h_0 ^ {1/h_0 })^{2(\frac {d_e(p_W+1)}{a})^3} \approx ((a/2\pi) ^ {2\pi /a })^{2(\frac {(1831^2 + 2^2)^{1/2}}{a})^3} $};
...
\node [anchor = east,color=red] at (2,12){$\hbar,G,G_F \rightarrow 52,255 (\approx \frac {3570}{8\pi e} Hz \rightarrow $ Lab ~(La = 442,9 Hz)  };
...
\node [anchor = west,color=red] at (-10,22){ $ R = \sqrt{\lambdabar_p\lambdabar_H} (WZ)^4 \approx \frac {\lambdabar_e}{f(\sqrt a)}(\frac {Z}{W})^{5\times 153} $};
...
\node [anchor = east,color=black] at (-5,7){\LARGE{MICRO-PHYSIQUE}};
...
\node [anchor = west,color=red] at (-31.5,10){$(ct_N/\lambdabar_e)^2 \approx R_e/2\lambdabar_e \approx D_{Eg}/l_P ~;~D_{Eg} \approx \sqrt {2R_T l_1} = 3570~ (l_1 = 1 METRE)   $};
...
\node [anchor = east,color=blue] at (-5,4){$a/138 \approx w_5/5 = 1-e^{-w_5} = \frac {hc}{5k_BT\lambda_{Wien}}$ };
...
\approx (137/2\pi d_e)^3\approx 137 (4/3)^{2/3} (137a)^{1/2}/2 $. 
Le rayon du Cosmos $R_C = R_e^2/2l_P$ vient de son rayon hol. réduit $R_e = (r_e = \lambdabar_e/a)^3/l_P^2$ lié à R par $R_e = R \times (u = Hp/a^3 \approx e^{2/e^2} ) = 2\hbar^2/G(m_N=am_e)^3$~;}}}

%~ $(\frac {2}{\Phi} )^{2 \times 209} \approx (2 \sqrt{10}/3\pi \approx (a/137g_3)^2 ) R_e \approx  WR/Zd_e^2 $~ (7ppm); $u_n = n_t^2/a^3 \approx \frac {aa_sZ\sqrt \beta}{2\times 137F } $ et $u_{p-1} \approx \Phi_{p-1}^2/2$ où $\frac {PO_B}{O_M} \approx \Phi_{p-1}^{10}; \Phi_0^{8} = 48 \approx \Phi^8 + 1  $. 
Le nb. d'Eddington $N_{Ed} = 136 \times 2^{256} \approx (3/10)M/m_n$, d'où $n_t = m_n/m_e \approx 1838.683659 \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). $f(18)/ \pi \approx$ le Nb Prem. d'Euler et le rapport thermique, relié aux points triples de $H_2, O_2, et H_2O$. }}}



%\textbf{Le cardinal du Monstre $O_M$ définit doublement la constante électrique $a \approx 137.035999099 $, (mesure 0.15 ppb), avec $a_s = a_w/2\pi (Hp)^{3/2}$}, $d_e \approx 1.001159652$. La période de Kotov vérifie $t_{sp}/t_K \approx O_M/\sqrt2 \approx e^a/F$ et $ct_K \approx d_{Soleil-Uranus}$. 
$f(29) \approx N_{Ram}^4 = e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-g_Z}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm) (\beta = 1/(H-p) $; n=12 confirme la symétrie d'Eddington Proton-Tau, hors Modèle Standard. 
\textbf{Pour n = 1836, la fonction de partition de Ramanujan $RAM_n= e^{\pi \sqrt{2n/3}}/4n \sqrt3$ confirme $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (2 ppm), lié à $p^{p^2}\approx (a^2)^{a^3}$}. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes du Monstre est lié à $a^a$ et au volume du Cosmos.}}}

%$f(14) = f(10)^2  \Rightarrow g_Z = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 \Rightarrow$, boson de Higgs = $495^2 ~ (10 ppm)$, lié à la constante d'Atiyah $\Gamma = \gamma a/\pi \approx a^{a/209}$, ;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/Gm_e;~ P=m_P/m_e;~H = m_H/m_e \approx 1837.152646~;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652 = (274\Gamma)^{3/2} \approx 2\pi a (665\times666)^{1/2}-3;FH^{(0)}/666p_G \approx (WZ sin \theta)^{1/2} = 6(139\times 138 +1) = 6N_0^{\prime} ~ (ppb) $~ seul nb commun, avec $N_0 = 43$, aux suites A235392 et A267504 $;  j= 8\pi^2/ln2;~ \Phi = (1+\sqrt5)/2$ (Nb d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nb d'Argent). \textbf{$m_{bc}$ est centrale}. \textbf{Le Neuron $ t_N = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ vérifie $Ct_N / \lambdabar_e \approx 5^2 f(30)$ et est lié à la seconde via 3570 liant le mètre au rayon terrestre $R_T$}. Couplages de jauge : $g_1 = Zsin\theta/H^0; g_2 = W/H^{(0)};   g_F \approx \frac {F}{H^{(0)}}; g_3/g_2 = g_0/g_1 \approx 6/ \pi \approx \Phi^{4/3};x_0 = g_0(1+g_1/g_2); \pi_a^2 = a^2-137^2 \approx aW/\sqrt {150} Z ; g_3 \approx 2/\Phi \approx 2/ln(2\pi_ag_3);  w_5 = hc/k_BT \lambda_{Wien} = 5(1-e^{-5})\approx 5a/138 ; WZ/N_0^{\prime} = N_0^{\prime}/sin\theta \approx (137/a)^2H^{(0)}/(3 \times N_0/2^7)^{1/2}$.} }}
