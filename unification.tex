...
{\color{red} Contrary to old tradition, the present science shows separated disciplines. In particular, the Number Theory does not play any role in Physics. Even the String Theory does not connect with Reality. Worse, the two pillars of Physics does not agree on the real status of space-time, and the standard model is incapable to define the values of about 30 "free parameters". Adopting the Optimal Computation Hypothesis, this article connects the Number Theory with a gravitation-reunified Physics implying chemical, biologic and cristallographic parameters untill 0.1 ppm precision. The superstring dimension 11 is identified to the first number of the base 3 Mirimanoff primes couple, while the second number of the base 2 Wieferich primes couple defines the fine-structure constant within its 0.15 ppb indetermination. The Euler maximal suitable number plays a central role, showing the connection with the generalised Riemann conjecture. This confirms the Eddington's Fundamental Theory, in particular the non-standard symmetry proton-tau. The running number of the string dimension series of the Topological Axis is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, and leads to stringent predictions.  } 
...
The general tendency of the \textit {formalist mainstream} has been to neglect such correlations, invoking either the hasard or a multiplicity of Universes (Multiverse hypothesis). However, the Large Number Correlations of Eddington, are now confirmed and extended to a Bott octonion sequence by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory, which was discarded because it involves a tachyon. This was considered an advantage, to explain the non-Doppler quasar oscillation.
...
This article recalls the application of the Holographic Principle to the PHOB cosmology (section 2). Then a degenerate form, the Holic Principle is applied in Section 3, which shows how the mass concept introduces a symmetry between gravitation and quantum physics in the Keplerian Diophantine equation. This connects the Bohr’s orbits with the PHOB model, showing up the mean DNA bi-codon mass which connects to 0.1 ppm with the Topological Axis. The local c-observable base 2 Universe appears as a gauge boson in the base 3 Tachyonic Cosmos with massive photon and graviton. This induces a scalar boson mass tied to the geometric extension of the Weinberg triangle, correlating the three gauge couplings U(1)-SU(2)-SU(3) with the gravity constant $G$.
...
{\color{red}The Present Physics faces a dramatic crisis : its two pillars are mutually incompatible. In particular, the principal hope of theoretical physics, the Holographic Principle is not applicable to the usual 4D space-time, due to the official temporal variation of $R$, the Hubble horizon radius. According to Schwarz : \textit {"this turns out to be surprisingly difficult"} \cite{Schwarz}. This article definitely prove its invariance, by showing 55 simple formula, with 8 in the 0.1 ppm domain. 
