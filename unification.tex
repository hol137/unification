...
\node [anchor = east,color=black] at (-8.25,10.250){~~~~~~\LARGE{MICRO-PHYSIQUE}};
\node [anchor = east,color=black] at (-8.25,8.250){$Y=\lambdabar_e/\lambdabar_{MICRO}$};
\node [anchor = east,color=red] at (22.,22.0){$RAMANUJAN(\sqrt{pn_t})\approx P^2/4\pi \approx (f(a^{2/3}) \approx f(2p/(a+1)  \approx a^{a^2/n_t} \approx n_t^{n_t/(a+1/2)} \approx e^{32\pi} $};
\node [anchor = east,color=red] at (22.25,-20.250){\LARGE{MACRO-PHYSIQUE}};
\node [anchor = east,color=red] at (22.25,-22.250){$Y = \lambdabar_{MACRO}/\lambdabar_{e}$};
