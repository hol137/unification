...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Euler-Napier Ct. and opt. nb. $e_{n+1}=e^{n}$;$e_4^{1/aa'}\approx (9/2)^{a'} \approx P^4 sin^2 \theta $~ ; $a' = a-1 $  & $e$    & 2.718281828459 & $e_2/e_3^{1/4} \approx e^{-e^{e^{-e}}} \approx g_1$  \\    
    Geom. Ct. $\pi$ and $\pi_a \approx 6/(1/g_1 - 1)\approx 3/(1-g_1g_2g_3/6)\approx 3 + (q/(1 + 1/\Delta_{Ed}))^2/g_2$ & $\pi$    & 3.14159265358 & $\pi_a = (a^2-137^2)^{1/2}$\\    
    %$\pi$ fifth fractional term & $\pi_5$    & 292.63459101440 & "exact" \\ 
    Euler-Mascheroni Ct. $\gamma/\pi = \Gamma/a$ \cite{Atiyah} $\approx ln\Phi / \Phi^2 \approx ln(\zeta (3))$ & $\gamma$    & 0.57721566490 & $\Phi^{\pi/\gamma} \approx e^{\Phi^2}\approx a/10$ \\ 
    Golden ratio and Black Body Photon Constant $\phi = \lambda/l_{ph} = (16\pi \zeta(3))^{1/3}$ & $\Phi\approx \phi g_2^2$  & 1.61803398874 & $\zeta(3) \approx 1.20205691$ \\ 
    ...
    Planck ratio $m_P/m_e \approx e_4^{(a-1)^2/4} \approx (3/\sqrt2)^{a/2} \approx (1/sin \theta)^{ad_e/2}\approx 2^{2^6}(a/4\pi)^3 p_t\beta^2/n_t$ &$ 10^{-22} P$   & $2.389015907 $ & ppb  \cite{Sanchez2} $P\approx a\tau^2Z^3/\pi\sqrt6 $  \\   
...
A-dimens. El. charge $q \approx 3/\pi^2 \approx (3/10)\ sqrt(6F)/p_{00} \approx (6^2/\beta d_e)^{-1/3}\approx (6\mu/137g_0\Gamma)^2$  & $q\approx e^{16a/p}$ & 0.30297311 & $6 \approx 2^{137/53}$ (indian music) \\
