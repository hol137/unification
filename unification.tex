...
Since the Poincare-Hilbert opposition, mathematians separated in intuitionnists and formalists. In particular, at a Solvay Congress, Poincare convinced at last the reticent communauty (including Planck) that there is no means to escape the real quantization of the interaction field-matter \cite{Poincare1}. Poincare explained also that the cosmology cannot be founded on differential equations, because of the involved "free parameters" in an \textit{unique} Univers \cite{Poincare2}. Las, the mainstream has followed Hilbert and his differential equations, leading nowadays to a blockage of the standard 6 "free parameters" cosmology, with its "mysterious" so-called "dark energy" with the relative density 0.7, by respect to the critical one \cite{Zyla}. 

The situation is even worse in the particle physics, with about 20 "free parameters". The Table 1 presents the main ones, and the Table 2 recalls the main physical constants \cite{Zyla}. For specifying some values, an Optimal Correlation Principle has been applied \cite {Sanchez2}, interpreting the "free parameters" as computation bases in a Computing Cosmos. 

Also, the standard Superssymetry model fails completely, since no confirmation comes from the CERN super-collisionner. \textit{This article shows that a number of symmetries have been overlooked, in particular a meson eta-lepton tau symmetry, which could enter a future Corrected Supersymmetry, and also the Eddington's proton-tau symmetry, which could enter a future "Intersymmetry", connecting particles of same spin. It has been forgotten that Eddington predicted the tau fermion, 35 years before its surprising discovery, with a fair estimation of its mass. Starting from an electron-proton symmetry Eddington deduced a "mesotron-heavy mesotron" one. This article rehabiltates these symmetries, which implies the last ideonal Euler number, connected with he Riemann hypothesis.}

The table 1 shows the evident pertinence of the most famous prime number in Mathematics, the Lucas Number, which enters the last term of the Combinatorial Hierarchy \cite{Bastin} \cite{Noyes}. It defines a gravitational proton-mass ratio $p_G$. This article shows that it enters a 0.1 ppm correlation with the Higgs-electron mass ratio $ H^{(0)}= 495^2$ \cite{Sanchez2}. This is on the way to resolve the "hierarchy" problem, the huge gap between gravitation and electroweak couplings. 
