...
\label{tab:5:table5}
  ...
  L'identification $m_G = m_{\hbar} = $m_{bc}^{(0)}$ conduit au double de la longueur de Kotov, associée à la période non-Doppler des quasars de Lyuty, qui prouve la nature tachyonique du Monde, c'est-à-dire qu'il est régi par des signaux allant beaucoup plus vite que la vitesse-lumière, brisant le "tabou d'Einstein" qui a bloqué la Physique depuis un siècle. 
  
La nouvelle masse est celle du bi-codon d'ADN de pureté isotopique $m_{bc}^{(0)}= m_H \sqrt {m_n/m_p} $, impliquant le rapport de masse neutron-proton. L'ADN est donc en liaison tachyonique avec le Monde. 
