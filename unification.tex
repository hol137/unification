...
\node [anchor = text=black] at (-10.,34.75) {\small{Les longueurs caractéristiques suivent la loi: $exp(2^{d/4})$}};
...
\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les longueurs caractéristiques $Y$, rapportées à la longueur d'onde de l'électron $\hbar/ m_ec=\lambdabar_e$, suivent la loi $exp(2^{d/4})$. C'est la réunion de 8 relations holographiques imbriquées 2D-1D. L'imbrication micro-macrophysique explique le \textit{double} logarithme $y = ln(lnY)$ qui s'avère fonction de la série spéciale des dimensions de la théorie bosonique des cordes, laquelle s'identifie avec la série spectroscopique de spin 1/2 (lié à la dimension 2 de la corde), où $k$ est le nombre quantique orbital, $d = 4k + 2$, de $k = 0$ à $k = 7$, charactéristique d'une séquence d'octonions de Bott. L'Univers observable apparaît comme le boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Weak (k=3), Gluon (k=1). La dimension principale 26 des cordes bosoniques correspond à 12 fois le demi-rayon de Hubble $R/2$,très voisin du nombre de Lucas, tandis que le Topon correspond à $f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e $. Donc $f(30) = f(26)^2$ implique $R \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse moyenne qui apparaît dans la Super-Période $t_C$ qui fait intervenir la constante de Fermi $G_F$ et la densité critique $\rho_c$. De plus, la relation $M/m_n \approx (10/3)N_{Ed}$ implique $m_n/m_e \approx \pi \times 153^2/40$ (80 ppm). Le nombre $f(18)/ \pi$ est très voisin à la fois du Nombre d'Euler et du rapport thermique principal. Le cardinal du Monstre $O_M$, qui apparaît dans la relation $f(24) = (f(12))^8$ implique $H^{16} \approx O_M \times \phi_0^8$, avec $\phi_0^8 \approx 48$, montrant que $H = m_H/m_e$ est base de calcul. La période de Kotov $t_K $ caractérise l'oscillation non-Doppler, donc tachyonique, des quasars, liée à $t_C$ par $t_C/t_K \approx O_M/\sqrt2 \approx e^a/F$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = 2\hbar^2/Gm_N^3$. Le couple Proton-Tau, symétrie inconnue du Modèle Standard, a été prévu par Eddington. La constante $e^{\pi \sqrt{163}}$ et la partition de Rama:3nujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ pour n = 137 jouent un rôle central illustrant la relation géo-combinatoire $p^{p^2}\approx (a^2)^{a^3}$, et la relation caractérisant $a$: $ f(a^{2}\approx (4\pi/3) \sqrt{f(26)f(27)}$, et où apparaît le produit des ordres du Monstre et du Bébé-Monstre, voisin du nombre de photons dans l'Univers. De plus, $f(14) = f(10)^2$ implique $g = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 $, où $3570 = (2+3+5+7)(2\times 3\times 5\times 7) $ confirmant, à $10^{-8}$ près, le boson de Higgs $495^2$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah, où $a \approx e 137.0359991 $ est la constante électriqu; $~~p = m_p/m_e \approx 1836.152673;~~H = m_H/m_e \approx 1837.152646~~n_t= m_n/m_e \approx 1838.683661; j= 8\pi^2/ln2 \approx 113.9; \Phi = (1+\sqrt5)/2$ (Golden Number) ; $\phi = 1 +\sqrt2 $ (Silver Number) $\rho_c = 3c^2/8\pi R^2$. La masse du Bicodon d'ADN $m_{bc}\approx m_H \sqrt{m_n/m_p}$ apparaît comme centrale, à travers sa longueur d'onde $\hbar/$m_{bc}c$ }}
