...
Since the Poincare-Hilbert opposition, the mathematians have been divided between intuitionnists and formalists. In particular, Poincare insisted on the fact that the cosmology cannot be founded on differential equations, because of the involved "free parameters" in an \textit{unique} Univers. Las, the mainstream has followed Hilbert and his differential equations, leading nowadays to a blockage of the standard 6 "free parameters" cosmology, with its "mysterious" so-called "dark energy" with relative density 0.7, by espect to the critical one. 

The situation is even worse in the particle physics: there are about 20 "free parameters". The Table 1 presents the main ones, and the Table 2 recall the main physical quantities. For specifying some values, an Optimal Correlation Principle has been applied, interpreting the "free parameters" as computation bases. Also the superssymetry model fails completely, since no confirmation comes from the CERN super-collisionner. This article shows that a number of symmetries have been neglected, in particular the Eddington's proton-tau symmetry. But his Fundamental Theory has been disregarded 
 

So, the cosmology \textit {must} be founded on integral equations. In particular, it is well-known that the gravitational energy of an homogeneous ball of mass $M$ and radius $R$ is $ E_{pot} -(3/5)GM^2/R$, which is, for the critical radius $R = 2GM/c^2$, the energy $ E_{pot} = -(3/10)Mc^2$, which is exactly the opposite of the \textit{non-relativist} cinetik energy $E_{kin}$ of the recceeding galaxies in the steady-state model, which is characterised by the classical integral equation 

\begin{equation}\label{Eq2}
E_{kin} = - E_{pot} = (3/10) Mc^2 
 \end{equation}

Hence it is not surprising that it appears a kind of energy with complement relative density 0.7.{\textit So the deepest mystery of standard cosmology is the clearest evidence for its opponent model, the steady-state model. This would mean that the standard interpretation of observations, based on a differential formalism, is wrong.} 

This means also that a local theory, Relativity, was \textit {unduly} applied to cosmology. Indeed, this Theory is based on the concept of an inertial referential, which cannot be defined by theory, but is observationally evident: the Cosmic Microwave Background (CMB). In liaison with the star production of Helium, this thermal bath was predicted by the steady-state model. This was contradictory with the initial standard model, which was obliged to introduce the monstruous inflation step, which introduces as many problems as it resolves \cite{Sternhardt}. Moreover, one of the three conditions of Sakharov for the elimination of antimatter during the Big Bang is precisely the rupture of thermal equilibrium. So \textit {the CMB is the refutation of the Big Bang, but it was unduly presented as its confirmation.}
...
Every phenomena which cannot be explained by the standard model is not taken seriuosly, as the non-Doppler power oscillation of sveral quasars. This proves the existence of tacyion, which are a necessity, sinc the speed $c$ is far too small to connect a so vast Universe. Indeed, the simplest application of the primary method in Physics, the \textit {c-free} dimensional analysis, gives half the Hubble radius. \textit{the modern cosmology foundators has missed the most elementary calculation}.  

The Table 2 presents two adimensional nearby (0.3 \%) quantities $K_n$ and $K_E$ which, in the standard cosmology, are believed to follow different temporal evolutions. This is an argument in favor of a mixture of the steady-state model and the Bang one, since both the Cosmic Microwave Background and the Cosmic Neutrino background are invoked. Such a model has been proposed: the Permanent Holographic Oscillatory Bang (PHOB), where the matter is considered as a very rapid matter-antimatter oscillation, and dark matter a $\pi$-dephased oscillation \cite{Sanchez2}.

The general tendancy of the \textit {formalist mainstream} has been to neglect such correlations, invoking either the hasard or a multiplicity of Universes (Multiverse hypothesis). However, the Large Number Correlations of Eddington, are now confirmed and extended to a Bott octonion sequence by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory, which was discarded because it involves a tachyon. This was considered an advantage, to explain the non-Doppler quasar oscillation.
...
\section{The Holographic Principle, the Topological Axis and the \textit{invariant} Hubble radius}\label{AxeTopologique}

{\color{red}The Present Physics faces a dramatic crisis : its two pillars are mutually incompatible. In particular, the principal hope of theoretical physics, the Holographic Principle is not applicable to the usual 4D space-time, due to the official temporal variation of $R$, the Hubble horizon radius. According to Schwarz : \textit {"this turns out to be surprisingly difficult"} \cite{Schwarz}. 

Moreover, the standard model of particles in unable to explain the origin of about 20 "free parameters". This article shows they are connected with the Hubble radius.
 
Another hope of theoretical physics, the string theory, does not succed to connect wity Reality. This article rehabiltates this theory, especially its bosonic part, which was rejected for its implication of a tachyon. This article proves that the tachyonic hypothesis is a necessity.

} 

In practice, the present-day value for $R$ introduces a dramatic crisis. Despite an optimisation of six parameters, the standard $\Lambda CDM$ cosmologic  model leads to the present-day Hubble constant $ H_0 = c/R \approx 67.74 ~\pm~ 0.46$ km/s by Megaparsec \cite{Zyla}, while, since several years, direct measurements, using different methods, confirm a significant discrepancy. In particular, the latest measurement is $69.8 \pm 0.8$ \cite{Freedman}. 
...
% Critical density $3 c^2/ 8\pi R^2$ & $\rho_{cr}$  & $kg m^{-3}$  & $ 9.41197996 \times 10^{-27} $ & 0.3   \\
  ...
  %Official CMB temperature & $T_{CMBoff}$ & K & $2.7255(6)$ & $2 \times 10^5$ \\    
   ...
   CMB reduced wavelength & $\lambdabar_{CMB} = \hbar c/ k_B TT_{CMB} $ & m & $8.4007166 \times 10^{-4} $ & \cite{Sanchez2} \\
   CMB Wien wavelength & $\lambda_{CMB}/w_i $ & m & $1.0630825  \times 10^{-3} $ & \cite{Sanchez2} \\
   
   
   
   CMB photon length & $l_{phCMB} = \lambda_{CMB} (16\pi \xi(3))^{-1/3} $ & m & $1.345131  \times 10^{-3} $ & \cite{Sanchez2} \\
   
   
   CMB density energy & $u_{CMB} = (\pi^2/15)\hbar c/\lambda_{CMB}^4 $ & $J m^{-3}$ &  $4.1767647  \times 10^{-14} $ &  \\
   
   Radiation density energy & $u_{CMB+CNB} = y u_{CMB} $ & $J m^{-3}$ &  $7.0224862  \times 10^{-14} $ &  \\
   
   Critical Matter versus radiation ratio $\rho c^2/u_{CMB+CNB} $& $K_E$ & - &  \underline {12045.685} &  \\
   
   Square root of the ratio double CMB photon number/atomic number & $K_n$ &- & \underline {12087.731}  & Eq(7) \\
   
   
   
   Optimised CNB temperature $T_{CMB}/(11/4)^{1/3}$ & $T_{CNB}$ & K & $1.9455976$ &   \\
   
   Neutrino (CNB) reduced wavelength & $\lambdabar_{CNB} = \lambdabar_{CNB} \times (11/4)^{1/3}  $ & m & $1.176957 \times 10^{-3} $ & \cite{Sanchez2} \\
