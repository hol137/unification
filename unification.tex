...
Combined R23W: $u'_d = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & $v_{12}$    & $ \underline {1831} $ & 30D to 12D \\   
 R23W for $d$ = 8:  Eddington's formula\cite{Eddington}: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & $v_{8}= 495 + 496$ \\      
  ...
  $u_{2d} = 1 + 2d(4d+1)$ Bisection of odd central polygonal numbers for \underline {d = 4}  & $a_{00} $    & \underline {137}  & OEIS (A188135) \\
   ...
   Opt.U(1) coupling $Zsin\theta /H^{(0)} \approx e^{-e^{e^{-e}}}  \approx (\gamma/y)(n_t/1837)\approx p_W \sqrt 2/H f(2)$ &  $g_1$ & 0.3436256 & $f(2) = e^{\sqrt 2}$\\
...
\begin{equation}
 \left\{
   ...
   tg \theta =  g_1/g_2 = g_0/g_3   ~~~~~\rightarrow~~~~~    1/lng_3 \approx 5 + 1/137 = 2\times7^3/137\\
   g_1 cos \theta = g_2 sin \theta = (4\pi_q/a)^{1/2} \approx y\Pi_+/2d_e\sqrt F  ~~~~~\rightarrow~~~~~  3^{11-1}\approx 6(2\pi_q)^5 \approx \pi a^2\\
