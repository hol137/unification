...
{\color{red} The unified Pythagoras Number Natural Philosophy is replaced now by a science showing separated disciplines lacking connexion with the Number Theory. The two pillars of Physics does not agree on the real status of space-time, and the standard physics contains about 30 "free parameters". In the frame of the Permanent Sweeping Holographic Oscillatory Bang model (PSHOB), this article uses the Lucas Large Number to connect gravitation and the gauge couplings. Through the Holic Principle, the mass concept induces a symmetry between the Newton and Planck constants. Using a cellular automaton series, the main dimension 30 of the Topological Axis reduces to unity, through 12D and 4D compatifications, debunking the String Theory. This 1D Holography means a "Sweeped" Absolute Space, tied to the microwave background, justifying at last the local time relativity for a moving object. The Euler maximal ideonal number, connected with the generalized Riemann conjecture, is tied to the proton-elecron mass ratio. The supergravity dimension 11 identifies with the first prime of the base 3 Mirimanoff couple, in connection with the arithmetical monster 137, while the second prime of the base 2 Wieferich couple defines the fine-structure constant within its 0.15 ppb indetermination. This confirms the Atiyah approach and the Eddington's Fundamental Theory, in particular the outstanding non-standard symmetry proton-tau. The Wieferich couple strongly suggests an eta-tau symmetry, tied to basic economic numbers. These overlooked symmetries explain the failure of the present Supersymmetry. The Economic Computation Principle and the Holic Principle justify at last the origin of the fermion masses. This implies physical, chemical, biologic and crystallographic parameters reaching 0.1 ppm precision. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero-mass Particle, and leads to stringent predictions. } 
...
where $ m_e^{(red)} = m_e m_p/m_H$ is the formal reduced electron mass in the hydrogen atom. \textit {Such a pertinence of the Eddington Number means that the Number Theory enters in Physics: this is a goal of the present article to confirm this}. According to Russel "The most surprising in the modern science is its come back to Pythagorism"\cite{Alcina}. Indeed, the formalist mainstream rather followed Dirac when he interpreted the Cosmic Large Number correlation as age-varying, with a gravitation constant $G$ varying with time. It was forgotten that Poincare clearly specified that Physics is not possible with varying constants \cite{Poincare2}.
...
\subsection{The Classical Universe Radius and the Cosmic Gravity-Microphysics connection}

The standard theory associates conservation law with symmetry. However, more generally, a conservation law can be seen as the result of a computation. So, the main hypothesis is that the Cosmos, considered as a black-hole of radius $R_C$ is a super-computer, a very controversial domain \cite{Sanchez2}. In particular, Y. Ng \cite{Ng} introduces wrongly a "quantum foam" of dimension $(Rl_P^2)^{1/3} \approx 10^{-15} $ m, a length independantly found through the above Economic Computation Principle \cite{Sanchez5}. With the above hypothesis of an \textit{invariant} Hubble radius $R$, this can be rather associated with the \textit{invariant} electron classical radius. Hence, the "classical radius" of the Universe was defined as \textit{the radius which is the simplest elimination of $c$ between the classical electron radius and the Planck length formula. Indeed the speed $c$ is dramatically too small to inter-connect a so vast Cosmos}: 

\begin{equation}\label{Eq4}
R_e = 2r_e^3/l_P^2 = 2 \hbar^2/Gm_N^3 ~~~~~~~~~~~~ M_N = m_P^4/m_N^3
 \end{equation}
 
where $M_N$ is the associatd critical mass. The factor 2 comes from the Schwarzschild horizon formula $R_e  = 2GM_N /c^2$, implying the Nambu mass $m_N = am_e$, central in particle physics \cite{Nambu}. \textit{This considers the mass concept as primordial, specially the Panck mass $m_P$, excluding any Zero mass particle concept, at complete opposition with the standard approach}\cite{Sanchez2}.  

Applying again the ECP, we introduce the "Weinberg-Sanchez" natural geometric extension of the Weinberg triangle \cite{Taylor}, $1/g_0 = 1+g_1^2 +g_2^2 = 1 + (Z/H^{(0)})^2$. With the BE-Higgs scalar boson mass ratio, by respect to electron:  $H^{(0)} = 495^2$ (125.208 GeV)\cite{Sanchez2}, the radius ratio $R_e/R$ obeys the following $10^{-7}$ precise relations, involving $G$ through $p_G = P/2^{127/2}$ (from the Combinatorial Hierarchy \cite{Bastin}) and the Wien factor $w_i$. The ECP suggests to identify $1/g_0$ with $g_2/g_1g_3$ where $g_3$ is the SU(3) coupling, while $g_1 = W/H^{(0)}$ is the U(1) coupling and $g_2 = Z sin~\theta/H^{(0)}$ the SU(2) coupling, with $cos\theta = W/Z$ (the small difference between $g_0$ and $g_2$ induces formula in the Hubble and Cosmos Tables):  
...
In this manner, in conformity with the standard model, $g_3$ is connected with the strong coupling constant, but implying the Wien factor, whose pertinence is not reckognised in the present standard model. There is a direct confirmation, precise to 0.1 ppm, of the CMB temperature invariance:
...
In any Diophantine equation, this Holic Principle allows to discriminate a temporal ratio $T$, acting by its square, from a spatial ratios $L$, acting by its cube (due to the 3D space). Indeed, \textit {the simplest Diophantine Equation, which implies a 2-dimensional Time}, $T^2 = L^3 = n^6$ is the Diophantine form of the third law of Kepler, and implies: $L_n = r_n /r_1 = n^2$ (the Bohr's orbit law) and $T_n = t_n/t_1 = n^3$. Hence, with $v_n = r_n/t_ n$:
...
So, the PHOB Cosmology is tied to the couple $G,\hbar$, while the classical quantum theory uses in fact the "photonde" couple $\hbar,c$, and the gravitation theory the "gravitonde" couple $G,c$. These three couples define the "Trihedra of Constants"(Fig. 2).

\textit{These neologisms 'photonde" and "gravitonde" are introduced to recall that only waves propagate. The overlooking of this fact is a main cause of misinterpretations in quantum physics.}
...
\textit{Note that 3 is the optimal integer base, the closest integer to $e$} \cite{Hayes}. This is tied to the economic functional definition of $e$:

\begin{equation}\label{Eq13}
x^{1/x} ~~maximal~~ for~~ x = e
 \end{equation}

It is foressen that future computers will use this base 3.
...
%\label{tab:4:table4}
  \hskip-0.0cm\begin{tabular}{llllllllllllll}
    \toprule
    \multicolumn{14}{c}{Table \ref{tab:4:table4} : Number of Crystallographic Ponct. Sym. ops, Euler ideodal Numbers and Rule 23 Wolfram series $u_d$}                  \\
    \cmidrule(r){1-14}
    ...
    $K_d$ (positive $PSO_{Cr}$)  & 1 & 5 & 5 & 19 & 19 & 59 & 59 & 165 & \underline{165} & 419 & 419 & 1001& 1001 \\
     ...
     13 final Euler ideonal nbrs  & \underline {312} & \underline {330} & 345 & 357 & 385 & 408 & 462 & 520 & 760 & 840 & \underline{1320}  & \underline{1365} & \underline{1848}\\
...
Note that, while $137 = l_{16}$, the $16^{th}$ Lazy Caterer number (maximal number of zones in a plane defined by n straight lines), $11 = l_4$ and $4 = l_2$. Moreover, with the Rule 23 cellular automaton Wolfram series $u_n = 1 + n(2n+1)$, and its combined form $v_n = u_{2(d+2)}$:
...
One notes  \textit {an 0.1 ppm relation between $p_{W2}$ and the supersymmetric electron-proton-neutron triplet}:
...
\begin{equation}
 \left\{
    ...
    p_{W1} p_{W2} \approx e^7 \times e^{3e} \approx  e^{e^e} = E\\
         ...
         p_{W2} = (K_{p_{W1}} + K_{p_{W1}-1} + K_{p_{W1}-2})^2 - 6 \approx 4a \sqrt {p_rn_t/d_e} \approx 4 p_{W1} p_{W2}/ln(p_{W1} p_{W2})     
