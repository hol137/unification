...
Thus the crystalographic numbers contain the weak constants F/a, through a special value of $\pi$ involving the weak angle $\theta$.

\section{The $4^{th}$ Weigel number and the coupling $g_1$}

The number $165 = K_8 = K_9 = K^-_{9} = K^-_{10}$ is very singular because it is a non-trivial binomial number, equal to $\sqrt {H^{(0)}}/3$, where $\sqrt {H^{(0)}} = 495$, another non-trivial binomial number, is the square root of the Higgs/Electron mass ratio, and the third "coperfect" number, the antecedent of the perfect number 496 which is the dimension of the string group SO(32). 

The crytallographic numbers are associated to the unit circle, which characterizes the U(1) group. So there must be a connection with the $g_1$ coupling. Indeed: 
...
\section{The $8^{th}, 10^{th}$ and $12^{th}$ Weigel numbers}
Among the Weigel numbers, only the $8^{th}$(165) and the $12^{th}$ (1001) are non-trivial binomial numbers encading the $81^{th}$ prime number $K_{10} = 419$. It is conjectured that these are the only two such cases. Taking account ot the above paramount importance of $K_8$, relations involving between these two numbers are looked for. Indeed: 
