...
Combined R23W: $v_d = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & $v_{12}$    & $ \underline {1831} $ & 30D to 12D \\   
 R23W for $d$ = 8:  Eddington's formula\cite{Eddington}: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & $v_{8}= 495 + 496$ \\      
  ...
  $u_{2d} = 1 + 2d(4d+1)$ Bisection of odd central polygonal numbers for \underline {d = 4}  & $a_{00} $    & \underline {137}  & OEIS (A188135) \\
    ...
    Optimized series with the second (last ?) Wieferich prime $137 + 1/28 + 1/p_{W2}$  & $a_0$    & $\underline {137.035999104}$ & $p_{W2}$ = 3511  \\
...
\end{equation}
\textit {This defines $a$ in its 0.15 ppb determination (Table 2)}. The analysis implies the above relation $g_1g_3 = g_0g_2$:
\begin{equation}
p_{495} = 3511 + 28 = 3539 \approx 419 a_s n_t/p_W \approx  \eta/q \approx 2\pi(9\mu) \approx 495 g_1p_G/g_2a \approx 495 g_3 \sqrt a/2 \approx g_0 (a^{5/2}/p_t)^{1/2}
\end{equation}
