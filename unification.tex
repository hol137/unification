...
\node [cross,anchor = east,color=red] at (3.15,3.15){$RAMANUJAN(137)\approx a \sqrt{a_G}\approx f(18) (R_e/R)^2$};
...
\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{Les principales longueurs caractéristiques $Y$, rapportées à la longueur d'onde Compton de l'électron $\hbar/ m_ec=\lambdabar_e$, suivent la loi $exp(2^{d/4})$. C'est  la réunion de 8 relations holographiques imbriquées 2D-1D. L'imbrication micro-macrophysique explique le \textit{double} logarithme $y = ln(lnY)$ qui est fonction de la série spéciale des dimensions de la théorie bosonique des cordes, qui s'identifie avec la série spectroscopique de spin 1/2, où $k$ est le nombre quantique orbital, $d = 4k + 2$, de $k = 0$ à $k = 7$, charactéristique d'une séquence d'octonions de Bott. L'Univers observable apparaît comme le boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Weak (k=3), Gluon (k=1). La dimension principale 26 des cordes bosoniques correspond à 12 fois le demi-rayon de Hubble $R/2$,très voisin du nombre de Lucas, tandis que le Topon correspond à $f(30) \times 153^2$ où $12\times 153 = 1836 \approx mp/me $. Donc $f(30) = f(26)^2$ implique $R \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse moyenne qui apparaît dans la Super-Période $t_C$. De plus, la relation $M/m_n \approx (10/3)N_{Ed}$ implique $m_n/m_e \approx \pi \times 153^2/40$ (80 ppm). Le nombre $f(18)/ \pi$ est très voisin à la fois du Nombre d'Euler et du rapport thermique principal. Le cardinal du Monstre $O_M$, qui apparaît dans la relation $f(24) = (f(12))^8$ implique $H^{16} \approx O_M \times \phi_0^8$, avec $\phi_0^8 \approx 48$, montrant que $H = m_H/m_e$ est base de calcul. De plus, $f(14) = f(10)^2$ implique $g = Z/495^2 \approx 153/(2 \times 3 \times 5 \times 7) \approx 1838.5\sqrt2/3570 \approx (4\pi 495^2 W/a_w)^2/3 \approx (9/10)^3 p/\sqrt{n_tH}\approx \mu^{-\mu/\tau}/\beta $ dans $pF\approx (gH)^3/(\mu/a)^2$, confirmant le boson de Higgs $495^2$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah, où $a$ est la constante électrique 137.036. La période de Kotov $t_K $ caractérise l'oscillation non-Doppler, donc tachyonique, des quasars, liée à $t_C$ par $t_C/t_K \approx O_M/\sqrt2 \approx e^a/F$. La masse de Nambu $m_N = a m_e$ correspond au rayon holographique réduit du Cosmos $R_e = 2\hbar^2/Gm_N^3$. Le couple Proton-Tau, symétrie inconnue du Modèle Standard, a été prévu par Eddington. La partition de Ramanujan $e^{\pi sqrt{2d/3}}/4d sqrt3$ joue un rôle central.
}
