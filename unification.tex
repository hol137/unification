...
La formule générale $r_1$, basée sur la symétrie des constantes fondamentales $\hbar$ et $G$ s'identifie à la formule "3 minutes" donnant le demi-rayon de l'Univers observable pour l'identification $m_G = m_e, m_{\hbar} = \sqrt {m_pm_H}$ , où $m_e$ est la masse de l'électron et $\sqrt {m_pm_H}$ la moyenne des masses proton-hydrogène. C'est l'aboutissement du modèle considérant l'Univers comme une molécule géante d'Hydrogène, composée d'un atome d'Hydrogène, d'un proton et d'un électron. Ce rayon de l'Univers est également donné par l'Axe Topologique pour la valeur $k = 6$, correspondant à la dimension bosonique des cordes $n = 26$ : $R \approx g(6)\lambdabar_e $, où $g(6) = e^{2^{6+1/2}}/6 $. 
  
  
Le remplacement de $\sqrt {m_pm_H}$ par la masse de Planck $m_P$ donne la longueur d'onde réduite de l'électron $\lambdabar_e$, qui est l'unité de longueur privilégiée dans l'Axe Topologique. Le rayon de Bohr s'obtient alors en remplaçant $m_P$ par $m_P/\sqrt a$, où $a= 137.0359991$ est la constante électrique. En conservant $m_{\hbar} = \sqrt {m_pm_H}$, mais en remplaçant $m_e$ par $m_P a^3$, on obtient la longueur d'onde de Wien du rayonnement cosmique de fond, identifié au rayonnement micro-onde qui fut interprété à tort comme la trace refroidie d'un Big Bang Initial. Cette longueur d'onde, rapportée à la longueur de Planck montre clairement que la base $\pi$ est utilisée, alors que c'est la base 6 pour la longueu de Kotov.    
  
  
  L'identification $m_G = m_{\hbar} = m_N$, où $m_N = a m_e$ est la masse de Nambu, conduit au demi-rayon holographique réduit du Cosmos $R_e/2$. L'identification $m_G = m_{\hbar} = m_{bc}^{(0)}$ conduit au double de la longueur de Kotov, associée à la période non-Doppler des quasars de Lyuty, qui prouve la nature tachyonique du Cosmos, c'est-à-dire qu'il est régi par des signaux allant beaucoup plus vite que la vitesse-lumière, brisant le "tabou d'Einstein" qui a bloqué la Physique depuis un siècle. La nouvelle masse est celle du bi-codon d'ADN de pureté isotopique $m_{bc}^{(0)}= m_H \sqrt {m_n/m_p} $, impliquant le rapport de masse neutron-proton. L'ADN est donc en liaison tachyonique avec le Monde.
  
Le rayon du Cosmos s'obtient avec l'identification $m_G = m_{bc}^{(0)}R_e/R $ et $m_{\hbar} = \sqrt{ m_{ph}m_{gr}} $, moyenne des masses du photone et du gravitonde, correspondant respectivement aux 24 dimensions transverses de la théorie des cordes et aux 26 dimensions bosoniques. La base tilisée étant 6, il y a liason directe avec $(l_{cc}/\lambdabar_e)^4$, le faceur corectif étant $R_e/R \approx e^{2/e^2}$, qui apparaît comme central dans le Cosmos numérique.

\textit{La masse du bi-codon d'ADN joue un rôle central, tandis que la simplicité des raisonnements et des propriérés arithmétiques montrent que l'Humain retrouve sa place centrale dans le Cosmos : le Principe Cosmo-Anthropique est confirmé.}
