...
\draw[color=blue] (A) -- (Y);
\node [anchor = text=blue] at (-8.,35.75) {\Large{AXE COSMIQUE} $ (4\pi^ 2/3) (R_c/r_H)^3 \approx a^a$};
\node [anchor = text=black] at (-10.,34.75) {\small{Les longueurs centrales suivent la loi: $exp(2^{d/4})$}};
...
\node [cross,anchor = east,color=red] at (4.25,4.25){NOMBRE PREMIER D'EULER $(2^{2^5-1}-1) \times \pi ~~ $};
...
\node [anchor = east,color=red] at (22.,22.0){$RAMANUJAN_{\sqrt{pn_t}}\approx P^2/4\pi \approx (4\pi/3)(f(26)f(27))^{1/2} \approx f(2p/(a+1)  \approx a^{a^2/n_t} \approx n_t^{n_t/a} \approx e^{32\pi} $};
