...
\section{Erratum}
\subsection{The Topological Axis, the Weak Bosons and the Weigel number $K_8 = K_9 = K^-_{9} = K^-_{10} = 165$}
...
showing that the weak force is tied to the connection $e^e \approx 15$, characteristics of the Topological Axis.
 ...
 The Weigel series (Table 7) shows up the transdimensionnal following propery: the number of positive operations is the same for dimensions 2n and 2n+1. Moreover, the number of positive operations in dimension $n$ is equal to the number of negative operations in dimension $n+1$. This implies that, for an odd dimension the number of positive and negative operations are equal. Among these numbers, 165 is both the number of positive point operations in dimensions 8 and 9, and also the number of negative point operations in dimensions 9 and 10. Another property of 165 is its non-trivial binomial character. Another case is $K_{13} = 1001$ : it is conjectured that these are the only two cases. The number 165 is very singular because it is equal to $\sqrt {H^{(0)}}/3$, where $\sqrt {H^{(0)}} = 495$, another non-trivial binomial number, and the square root of the Higgs/Electron mass ratio. It is suggested to call "coperfect" the antecedent of a perfect number. 
