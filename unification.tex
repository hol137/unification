...
{\color{red} Contrary to old tradition, the present science shows separated disciplines. In particular, the Number Theory plays a marginal role in Physics. Even the String Theory does not connect with Reality. Worse, the two pillars of Physics does not agree on the real status of space-time, and the standard model is incapable to define the values of about 30 "free parameters". Adopting the Optimal Computation Hypothesis, this article connects the Number Theory with a gravitation-reunified Physics implying chemical, biologic and cristallographic parameters untill 0.1 ppm precision. The superstring dimension 11 is identified to the first number of the base 3 Mirimanoff primes couple, while the base 2 Wieferich terminal prime defines the fine-structure constant within its 0.15 ppb indetermination. The Euler maximal suitable number plays a central role, showing the connection with the generalised Riemann conjecture. This confirms the Eddington's Fundamental Theory, in particular the non-standard symmetry proton-tau. The running number of the string dimension series of the Topological Axis is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, and leads to stringent predictions.  } 
