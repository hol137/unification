...
\label{tab:6:table6}
 ...
 %$ R_e~\lambdabar_H/g_3l_P2^{129/2} $  & 13.811978 & From $1/g_0 = 1+g_1^2+g_2^2\approx2a^3/pp_G$\\
 
 $ (\lambdabar_{p} \lambdabar_{H})^{1/2}~ a_w^{7/2}~ a/2\sqrt 5 $  & 13.811978 & From $(WZ)^8 \approx a^2a_w^7/20$\\
 
 %$ 2l_P (aZ/(\sqrt a_w +\pi_q/3))^2 2^{3\times 127/2} $ & 13.811977 & Liaison between Z and the electric charge $q$ \\
  
% $ R/\lambdabar_e  \approx 6H (2\pi_{hol} a^3)^5 $& 13.811977    &  Holographique, avec     $\pi_{hol} = 3 + 1/(7+1/(16+2/e^{2e}))$\\
  
 % $ R_N (sin\theta_Z)^2/ q$  & 13.81197676    & Identification, à partir de la charge $q$ précisant $qR/R_N = (sin\theta_Z)^2\approx 0.231188354$   \\
 
  $ \lambdabar_e ~(p_{hol}/p)^2~ E^{\sqrt a/2}$ & 13.81198 & Liaison between $E = exp(exp(e))$ and $p_{hol}^2 = 4a^3/3$   \\
...
The overwhelming connections between the HVW crystallographic series, the string theory and the sporadic groups, confirm that the pure mathematics must now pursue unification, by concentrating on the mathematical properties of physical parameters. Such connections between apparently separated matematical domains has been already introduced by Physics \cite{Conway}. \textit{In particular, research must concentrate on the Algebra of Eddington's E Numbers \cite {Eddington} \cite {Salingaros}, the Euler ideonal numbers and the Wieferich and Mirimanoff primes}.
...
\bibitem{Sanchez1} (1995) Sanchez F.M., Holic Principle, Entelechies, ANPA 16. Bowden K.G., 324-343.
...
\bibitem{Weigel} Veysseyre R., Veysseyre H., and Weigel D. (1992) Counting, types and symbols of crystallographic Point Symmetry Operations of space $E^{n}$ AAECC 5, 53--70  DOI: 10.1007/BF01196625 ISBN: 0938-1279.
\bibitem{Weinberger} Weinberger, P.  (1973) Exponents of the class groups of complex quadratic fields. Acta Arith. 22, 117-124.
\bibitem{Ribenboim} Paulo Ribenboim 1979 13 Lectures on Fermat's Last Theorem, Springer, 23, 152-153.
