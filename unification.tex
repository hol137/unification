...
\node [cross,anchor = east,color=red] at (4.25,3.75){ $ \frac {aW}{Z}\approx \frac {1831}{e^e} \approx \frac {p_G}{\sqrt{p/8d_e} } $};
...
%\node [anchor = east,color= red] at (34.5,23.5) {~~~~~~$ p \approx p_W = 6 \pi^5  $};
...
%\node [anchor = east,color=black] at (-5.25,10.250){\LARGE{MICRO-PHYSIQUE}};
...
\node [anchor = east,color=black] at (-7.25,4){\LARGE{MICRO-PHYSIQUE}};
...
\node [anchor = east,color=blue] at (22.75,-6){R/2 = \frac {\hbar^2} {Gm_em_pm_H} = \frac {G_Ft_K^2} {m_e \lambdabar_e^4}   };
