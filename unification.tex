...
Such a pertinence of the Eddington Number means that Number Theory enters Physics: this is the main goal of the present article to confirm this. According to Russel \textit{"The most surprising in the modern science is its come back to Pythagorism"}\cite{Alcina}. Indeedt, every formalist followed Dirac when he interpreted the Cosmic correlation as age-varying, with a gravitation constant $G$ varying with time.
...
This is shown in the Table 2, which presents two adimensional nearby (0.3 \%) quantities $K_n$ and $K_E$ which, in the standard cosmology, are believed to follow different temporal evolutions. This is a strong argument in favor of a mixture of the steady-state model and the standard Big Bang one, since both the Cosmic Microwave Background and the Cosmic Neutrino background are invoked. Such a model has been proposed: the Permanent Holographic Oscillatory Bang (PHOB), where the matter is considered as a very rapid matter-antimatter oscillation, and dark matter a $\pi$-dephased oscillation \cite{Sanchez2}.

The general tendancy of the \textit {formalist mainstream} has been to neglect such correlations, invoking either the hazard or a multiplicity of Universes (Multiverse hypothesis). However, the above Eddington Large Number Correlations are now extended to a Bott octonion sequence, by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory \cite{Sanchez2}. The latter was \textit {unduly} discarded because it involves a tachyon. Quite the contrary this is considered an advantage, to interpret the above non-Doppler quasar oscillation.
