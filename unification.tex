...
{\color{red} Contrary to old tradition, the modern science is characterised by an a mutiplicity of separated disciplines. In particulat the Number Theory does not play any role in Physics. Even the String Theory does not connect with reality. Worse, the two pillars of Physics does not agree on the real status of space-time, and the standard model is incapable to define the values of about 30 "free parameters". Adopting the Ultimate Quantization Hypothesis, this article connects the Number Theory with a reunified Physics, implying chemical, biologic and cristallographic parameters to 0.1 ppm precision. The base 2 and base 3 paricular cases of the Fermat Theorem and the uler suitable numbers play a central role in the debunking of the arithmetica origin oftheefree parameters. In particular, this leads to the fine-structure constant within its 0.15 ppb indetermination. This confirms the Eddington's Fundamental Theory, in particular the non-standard symmetry proton-tau. The running number of the string dimension series of the Topological Axis is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero mass Particle, and leads to stringent predictions.  } 
