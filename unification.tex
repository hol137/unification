...
\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{
Les rapports Y (/ électron) entourent la loi $e^{2^{d/4}}$, basée sur 8 relations holiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série spectroscopique $d = 4k + 2$, de $k = 0$ à $k = 7$: \textbf{Le Tableau Périodique (\underline {s}=2, \underline {p}= 6, \underline {d}=  10, \underline {f}= 14)  est étendu à une séquence d'octonions de Bott} }}
