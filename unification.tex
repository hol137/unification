...
This suports the Holic Principle : the mass ratios operate by their fifth power, which is also tied to the mean life of unstable particles. The number $417 = K_{10} -2$ appears directly in the following. 
 ...
 \left\{  
    \begin{array}{ll}
       ...
       \\
        ...
        \\
        e \approx (K_{10}/(K_{10}-2))^{210-1} ~~~~ \rightarrow ~~~~  K_{10}/(K_{10}-2)\approx F^5/Pa^3  (ppb)
 \\    
...
\section{The $4^{th}, 5^{th}$ and $6^{th}$ Weigel numbers}
Among the Weigel numbers, only the $4^{th}$(165) and the $6^{th}$ (1001) are non-trivial binomial numbers encadring the $81^{th}$ prime number $K_{5} = 419$. It is conjectured that these are the only two such cases. Taking account ot the above paramount importance of $K_4$, relations involving between these two numbers are looked for. Indeed: 
