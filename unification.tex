...
Since the Poincare-Hilbert opposition, the mathematians have been divided between intuitionnists and formalists. In particular, Poincare insisted on the fact that the cosmology cannot be founded on differential equations, because of the involved "free parameters" for an \textit{unique} Univers. The mainstream has followed Hilbert and his differential equations, leading nowadays to a blockage of cosmology (6 "free parameters"). The situation is even worse in the particle physics: there are about 20 "free parameters". The Table 1 presents the main ones, and the Table 2 recall the main physical quantities. For specifying some values, an Optimal Correlation Principle has been applied, interpreting the "free parameters" as computation bases.  

So, the cosmology must use integral equations. For instance, it is well-known that the gravitational energy of an homogeneous ball of mass $M$ and radius $R$ is $-(3/5)GM^2/R$, which is, for the critical radius $R = 2GM/c^2$, the energy $-(3/10)Mc^2$, which complement has density 0.7. Nowadays, a dark energy of relative density 0.7 is invoked, which seems mysterious, and leads to an intense debate. This would mean that the standard interpretation of observations, based on a differential formalism, is wrong.
...
The general tendency of formalists has been to neglect such correlations, invoking either the hasard or a multiplicity of Universes (Multiverse hypothesis). However, the Large Number Correlations of Eddington, are now confirmed and extended to a Bott octonion sequence by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory, which was discarded because it involves a tachyon. This was considered an advantage, to explain the non-Doppler quasar oscillation.
...
Also the superssymetry model fails completely, since no corroborration comes from the CERN super-collisionner. This article shows that a number of symmetries have been neglected, in particular the Eddington proton-tau symmetry.
