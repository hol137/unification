...
\node [cross,anchor = west,color=red] at (3.15,3.15){$\Rightarrow T_{CMB} \approx 2.73 ~Kelvin \approx T_{mam}/j$};
...
\textit{Note that 3 is the optimal integer base, the closest integer to $e$} \cite{Hayes}. This is tied to the economic functional definition of $e$, whose square interconnects the main musical ratios, using the primes 2;3;5, while 7/6, unknown in classical music, connects with $a$:
...
\textit{It is foreseen that future computers will use this base 3. Recalling that the conscious music is a multibase computation using only the three bases 2;3;5, it is foreseen that inconscious music or dream could use also the base 7. A symmetric use of theese 4 bases explain the role of the global base 210, so justifies the muon mass.} 
...
\end{tikzpicture}
\caption[Figure \ref{figure2}: The $\hbar-G-c$ Trihedra]{The Trihedra of Constants $\hbar Gc$.  The $c$-local visible Universe is a Cosmos bosonic "immergence"}
\end{figure}
