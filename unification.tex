...
\node [anchor = east,color=red] at (16,17.250){$\hbar,G, m_{bc}^3 \rightarrow 2l_K \rightarrow m_{photon} = \hbar/cl_K \approx m_{graviton} \times (a_w \approx \frac {f(26)} {f(24)})$};
...
\node [anchor = west,color=blue] at (20.,20.0){$R_{Hub} \approx \frac{P}{a_s (a+1)}(R_T \approx ct_1 /\Phi^8) $};
...
\node [anchor = west,color=red] at (20,22.5){ $ \frac {R}{\sqrt{\lambdabar_p\lambdabar_eH} = (WZ)^4  $};

%\node [anchor = east,color=red] at (0,24){Synthèse triconceptuelle : $\hbar,G, m_N^3 \rightarrow R_e/2$};
