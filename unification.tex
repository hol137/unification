...
Hence it is not surprising that it appears a kind of energy with complement relative density 0.7.{\textit So the deepest mystery of standard cosmology is the clearest evidence for its opponent model, the steady-state model}. This would mean that the standard cosmology interpretation is unduly complicated. 

This means also that a \textit{local} theory, Relativity, was unduly applied to cosmology. Indeed, this theory is based on the concept of an inertial referential, which cannot be defined by theory, but is observationally evident: the Cosmic Microwave Background (CMB). In liaison with the star production of Helium, this thermal bath was predicted by the steady-state model \cite{Hoyle2}. This was contradictory with the initial standard model, which was obliged to introduce the monstruous inflation step, introducing as many problems as it resolves \cite{Sternhardt}. Moreover, one of the three conditions of Sakharov for the elimination of antimatter during the Big Bang is precisely the rupture of thermal equilibrium. So \textit {the CMB is the refutation of the Big Bang, but it was unduly presented as its confirmation.}
...
Every phenomena which cannot be explained by the standard model is not taken seriously, in particular the non-Doppler power oscillation of several quasars and the sun \cite{Kotov}. This proves the existence of tachyon, which are a necessity since the speed $c$ is far too small to connect a so vast Universe. Indeed, the simplest application of the primary method in Physics, the \textit {c-free} dimensional analysis, gives half the Hubble radius. \textit{the foundators of the modern cosmology have missed the most elementary calculation}.  

It is said that the present epoch is marked by the domination of matter on radiation. This is true for the energy, but the reverse holds for the ratio of particles numbers, considered as a key parameter, called "Critical Entropy" (the factor 2 being the polarisation factor):

\begin{equation}\label{Eq2}
Mc^2 /(E_{CMB} + E_{CNB}) \approx \sqrt {2n_{ph}/n_H}~~~~~~~~~~with ~~~ n_{ph} = (4\pi/3)(R/l_{phCMB})^3    ~~~~ n_ H= M/m_H
 \end{equation}

This is shown in the Table 2, which presents two adimensional nearby (0.3 \%) quantities $K_n$ and $K_E$ which, in the standard cosmology, are believed to follow different temporal evolutions. This is a strong argument in favor of a mixture of the steady-state model and the Big Bang one, since both the Cosmic Microwave Background and the Cosmic Neutrino background are invoked. Such a model has been proposed: the Permanent Holographic Oscillatory Bang (PHOB), where the matter is considered as a very rapid matter-antimatter oscillation, and dark matter a $\pi$-dephased oscillation \cite{Sanchez2}.

The general tendancy of the \textit {formalist mainstream} has been to neglect such correlations, invoking either the hasard or a multiplicity of Universes (Multiverse hypothesis). However, the Eddington Large Number Correlations are now confirmed, with extension to a Bott octonion sequence, by the Topological Axis (Graph 3) which rehabilitates the bosonic String Theory. The latter was discarded because it involves a tachyon. Quite the contrary this is considered an advantage, to explain the non-Doppler quasar oscillation.
