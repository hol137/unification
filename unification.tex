...
The situation is even worse in the particle physics: there are about 20 "free parameters". The Table 1 presents the main ones, and the Table 2 recalls the main physical constants \cite{Zyla}. For specifying some values, an Optimal Correlation Principle has been applied \cite {Sanchez2}, interpreting the "free parameters" as computation bases in a Computing Cosmos. Also, the standard superssymetry model fails completely, since no confirmation comes from the CERN super-collisionner. This article shows that a number of symmetries have been neglected, in particular a meson eta-lepton tau symmetry, which could enter a future corrected supersymmetry, and also the Eddington's proton-tau symmetry, which could enter a future "intersymmetry" connecting particles of same spin.
...
\begin{equation}\label{Eq2}
R/2\lambdabar_H = \sqrt {M/m_e^{(red)}} = \hbar c / G m_e m_p 
 \end{equation}
...
\textit {The deepest mystery of standard cosmology is the clearest evidence for its opponent simplest model, the non-relativist permanent Newtonian model, proving undue complication in the standard cosmology.} 
...
\begin{equation}\label{Eq2}
Mc^2 /(E_{CMB} + E_{CNB}) \approx \sqrt {2n_{ph}/n_H}~~~~~~~~~~with ~~~ n_{ph} = (4\pi/3)(R/l_{phCMB})^3    ~~~~ n_ H= M/m_H
 \end{equation}
...
Gravitational proton ratio $P N_L^{-1/2}$ & $p_G$    & $ \underline {1831.531181} $ & ppb  \cite{Sanchez2}  \\
  ...
  Optimised SU(2) Gauge coupling $W/H^{(0)}$  &  $g_2$  & 0.6421390 & 100 \\ 
  
  Gravitational Coupling $p_tp_G/2a^3$ & $ g_0$  & 0.6534149985 & 1 \\
