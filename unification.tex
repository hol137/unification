...
{\color{red} The unified Pythagoras Number Philosophy is replaced now by a science showing separated disciplines lacking connections with the Number Theory. This article shows that the four basic primes 2;3;5;7 are the calcuation basis of the Cosmos, whose two symmetric combinaisons  explain at last the two leptons mass ratios. This corresponds to an Holic Principle inducing a symmetry between the Newton and Planck constants which confirms the PSHOB by adding the DNA bi-codon in the electron-based Topological Axis, whose dimension 30 reduces to unity through 12D and 4D compatifications, using a cellular automaton series which debunks the String Theory.




%The two pillars of Physics do not agree on the real status of space-time, and the standard physics contains about 30 "free parameters". In the frame of the Permanent Sweeping Holographic Oscillatory Bang model (PSHOB), this article uses the Lucas Large Number to connect gravitation, the Cosmic Microwave Background (CMB) and the gauge couplings. Through the Holic Principle, The fermion masses are unambigously tied to the four basic primes, which are cosmic computation bases. This 1D Holography means a Hol Sweeping in Absolute Space, justifying at last the Parity Violation and the local time relativity for a moving object. The Euler maximal ideonal number, connected with the generalized Riemann conjecture, is unambigously tied to the proton-electron mass ratio. The supergravity dimension 11 identifies with the first prime of the base 3 Mirimanoff couple, in connection with the arithmetical monster 137, the CMB, the Golden Ratio and the old chinese musical scale. The second prime of the base 2 Wieferich couple specifies the fine-structure constant within its 0.15 ppb indetermination. This confirms the Atiyah approach and the Eddington's Fundamental Theory, in particular the non-standard symmetry proton-tau. The Wieferich couple strongly suggests an eta-tau symmetry, tied to basic "economic" numbers. These overlooked symmetries explain the failure of the present Supersymmetry. This implies physical, chemical, biologic and crystallographic parameters reaching 0.1 ppm precision and excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero-mass Particle, leading to stringent predictions.}  
...
%\textit{It is foreseen that future computers will use this base 3.}
\textit{A symmetric use of these four bases explain the role of the global base 210, so justifies the muon mass.}
