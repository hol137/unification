...
\node [anchor = west,color=red] at (-12.,-12.) {Rayon de l'atome d'Hydrogène $\times ~ 2\pi/3$ };
...
\node [anchor = west,color=red] at (-4.5,-4.5){Longueur d'onde de Rydberg $\times ~3g$};
...
\node [cross,anchor = west,color=red] at (4.25,4.25){PHOTON THERMIQUE: $~~\pi \times \hbar c/k_BT_{C}$};
\node [cross,anchor = east,color=red] at (3.25,4.75){NOMBRE PREMIER D'EULER $(2^{2^5-1}-1) \times \pi ~~ $};
\node [cross,anchor = west,color=red] at (3.15,3.15){$~~~ T_{CMB} \approx 2.77 ~Kelvin \approx T_{mam}/j\approx T_{H_2}T_{O_2}/T_{H_2O}$};
...
\node [anchor = east,color=red] at (14.25,-22.250){$Y = \lambdabar_{MACRO}/\lambdabar_{e}$};
