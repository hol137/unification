...
\node [anchor = west,color=black!40!black] at (8.0,8.5){$f(20) \approx (5/3)^{2^6}/2~~;\pi_a = \sqrt {a^2-137^2} $};
\node [anchor = east,color=black] at (8.0,8.25){$\sqrt{m_Pm_F}/\pi_a sin\theta ~m_e \approx$};

\node [anchor = east,color=black] at (12.25,12.250){BOSON UNIFIANT $X_{GUT}$};
\node [anchor = west,color=blue] at (12.25,12.250){\LARGE{NEURON}};
\node [anchor = east,color=black] at (14.,14.){$\pi^2 f(23) \approx \frac {l_P}{\lambdabar_e} = \frac {m_P}{m_e}$};
\node [anchor = east,color=blue] at (14.,14.){$\pi^2 f(23) \approx \frac {l_P}{\lambdabar_e} = \frac {m_P}{m_e} \approx MASSE OVOCYTE/m_e $};
