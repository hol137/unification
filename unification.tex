...
\node [anchor = west,color=black!40!green] at (21,0.0) {\Large{$\times~2R_e/R$}};
...
\node [cross,anchor = east,color=red] at (4.25,4.75){NOMBRE PREMIER D'EULER $(2^{2^5-1}-1) \times \pi ~~ $};
\node [cross,anchor = west,color=red] at (3.15,3.15){$~~~ T_{CMB} \approx 2.77 ~Kelvin \approx T_{mam}/j$};
...
\node [anchor = east,color=red] at (28.25,26.250){$f(29) \approx$ (NOMBRE DE RAMANUJAN)$^4 e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm)$~~~~~~~~ };
...
\node [anchor = north] at (-20.55, 33.8) {$e^{e^{e^{e^{e^{-g_2}}}}} ~~~~~ 2e ~~~  f(32) = e^{2^{2^{2^{2}}}} \approx (R_C/\lambdabar_e)(\tau/n_t)^{128/3} ~~~~~~(4\pi^2/3)(R_C/r_H)^3 \approx a^a$};
