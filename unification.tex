...
\textit {The Monster Group is related to the CMB through the holographic term $e^a$ and the canonical ratio $2R/R_e \approx 1/g_0$}.
...
\begin{equation}
 \left\{
    \begin{array}{ll}          
           (\tau_0^2/\mu_0^3)^2 \approx \tau/p_W \approx n_t^2pp_W\tau/H^5 \\
           (p_t/n_td_e)(\tau/\tau_0 )^{137} \approx \sqrt2 p^3/a_s^2H^2(H-p) \approx \pi^{\pi}  
    \end{array}
\right.
\end{equation}
confirming to the ppb range the Koide tau value \cite{Koide}, where $n_t/p_t$ is the mass ratio neutron-proton. Moreover, the order of the Baby-Monster $O_B$ connects with the string factor $f(2)$:
\begin{equation}
 O_M^{2/3}/O_B \approx \mu + 2 \approx 3 f^3(2)  
 \end{equation}
