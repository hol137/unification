...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Neutron/Electron mass ratio ~ $n_t \approx (\pi^2{\color{blue}W_5}/4)^3 \approx  p_t + 1 +1/g_0 $  & $n_t$ & 1838.6836617  & 0.5 ppb \\     
...
Combined R23W: ${\color{blue}u'_d} = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & ${\color{blue}u'_{12}}$    & $ \underline {1831} $ & 30D to 12D \\   
 R23W for $d$ = 8:  Eddington's formula\cite{Eddington}: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & $v_{8}= 495 + 496$ \\      
  Combined R23W ${\color{blue}u'_1} = u_{2\times (1+3)}$ for \underline {d = 1: Hol Sweeping 1D Absolute Space}  & {\color{blue}$u'_1 $}    & $ \underline {137} $ & 12D to 1D \\
  $u_{2d} = 1 + 2d(4d+1)$ Bisection of odd central polygonal numbers for \underline {d = 4}  & $a_{00} $    & \underline {137}  & OEIS (A188135) \\
    ...
    Sum {\color{blue}u$'_1$} = $ u_1^2 + u_2^2$ implying usual 4D  and supergravity 11D    & $a_{00}$    & $\underline {137}$ & $u_1 = 4$; $u_2 = 11$ \\
...
\label{tab:3:table3}
 ...
 Critical density $3/ 8{\color{black}G}\pi Gt^2$ & $\rho_{cr}$  & $kg ~m^{-3}$  & $ 9.41197996 \times 10^{-27} $ & with $t = R/c$, $\rho_{cr}$ independent of $c$   \\   
  ...
  CMB wavelength $h c/ k_B T_{CMB}$ & $\lambda_{CMB}  $ & $m$ & $5.278325924 \times 10^{-3} $ & \cite{Sanchez2}  $\lambda_{CMB} \approx 2f(18)\lambdabar_{e}((a-136)a/137)^{1/2} $\\
   ...
   Radiation density energy $ u_{CMB}+u_{CNB}  = y u_{CMB}$ & $u_{rad}  $ & ${\color{blue}J ~m^{-3}}$ &  $7.0224862  \times 10^{-14} $ & \cite{Sanchez2} \\
