...
The Number Theory is placed at the heart of an unified Science, specifying the fine-structure constant within its 0.15 ppb indetermination. The four basic primes 2;3;5;7 are the calculation bases of the Cosmos, whose two symmetric combinaisons explain at last the two leptons mass ratios. The corresponding Holic Principle induces a symmetry between the Newton and Planck constants which confirms the Permanent Sweeping Holography Bang Cosmology (PSHOB). This implies the DNA bi-codon central isotopic mass, confirming the electron-based Topological Axis to 0.1 ppm. The c-observable base 2 Universe is the terminal boson of the base 3 Cosmos. The forgotten Eddington's proton-tau symmetry, and other hidden ones, renew the supersymmetry quest. This excludes the concepts of Multiverse, Continuum, Infinity, Locality and Zero-mass Particle, leading to stringent predictions. 
...
\begin{table}
\caption[Table \ref{CosmosTable}: 50 Cosmos radius formula]{50 formula for the Cosmos radius $R_C$}
\label{CosmosTable}
  \hskip-2.0cm\begin{tabular}{lll}
    \toprule
   % \multicolumn{3}{c}{}                   \\
 ...
 $ l_P ~ (\sqrt{a_w}(a)^2)^{16}  $ & 9.07568 & Computation with the nuclear couplings $a_w$ and $a_s$  \\
 
 
 $ l_P ~ \sqrt{1 + (sin\theta_1)^2}~a^{j/2}  $ & 9.07566 & Computation with the electric coupling $a$  \\
...
The mean nucleotide mass is $ (o_1 + o_2)/4 \approx 306.4032199$, close to $\pi^5 \approx 306.02$, the sixth part of the Lenz-Wyler proton-electron mass ratio \cite{Wyler} $6\pi^5$ showing a geometric property: it is the product of the area by the volume of a cube of side $\pi$. The mean DNA bi-codon mass  $m_{bc}^{(0)}/m_H = (6/4)(o_1 + o_2) \approx 1838.418122$ connects very precisely with main parameters and the central term $f(16)= e^{16}$ of the Topological Axis (150 ppb and 55 ppb):

\begin{equation}\label{Eq20}
m_{bc}^{(0)} \approx (m_H^2/m_e)(n_t/p_t)^{1/2}  \approx m_ee^8 a^{3/2}/d_e \sqrt 2
 \end{equation}
