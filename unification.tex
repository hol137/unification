...
\node [anchor = text=black] at (-15.,34.75) {\small{Les longueurs caractéristiques entourent la loi: $f_b = b^{2^{d/4}}$~ pour~ $b = e$}};
\node [anchor = text=black] at (-29.5,33.5) {\small{Unités: LONGUEUR: $\hbar /m_e c=\lambdabar_e = ct_e$ TEMPS: $t_e$ (Neuron et Super Période) MASSE :$2\pi m_N$ (Photonde et Gravitonde) }};
%\node [anchor = text=black] at (-20.,32.75) {\small{Unité pour le Neuron et la Super Période $t_{sp}:t_e$. Unité pour Photonde et Gravitonde: $2\pi m_N$}};
%\node [anchor = text=black] at (-15.,31.75) {$a \approx 137.0359991$ };
%\node [anchor = text=red] at (-25.,32.75) {\small{$4\pi^2/3$};
...
\node [anchor = east,text=red] at (31.75,31.9) {$R_C/l_P \approx 3570^{137/8}$~~\LARGE{COSMOS}};
...
\node [anchor = east] at (-20.,-20.0) {$GLUONS$};
...
\node [anchor = west,color=blue] at (-12.,-12.) {$3f(10) \approx 2\pi r_H$ (Rayon de l'atome d'Hydrogène) $\approx (\frac{2}{u})^{16} \approx (3570)^{16}/O_M$ };
...
%\node [anchor = west,color=blue] at (-3,-6){$f(\Delta_ Ed}/10) \approx $ Longueur d'onde de Rydberg 

\node [anchor = west,color=blue] at (-6.2,-6.3) {$f(\frac{\Delta_{Ed}}{10}) \approx  $~ Long. de Rydberg $\lambdabar_{Ryd} \approx l_P \sqrt{O_M}\approx \sqrt{\lambdabar_p l_0}  $  };
 
\node [anchor = west,color=red] at (-8.2,-8.3) {~~~~~~ $ \frac{l_0}{l_P} \approx \frac{RR_e}{(2ct_0)^2} \approx O_B ( \frac{2ct_0 }{2\pi R_T} = \frac{\Phi^8 }{\pi}) \approx \frac{O_M}{\sqrt{a_G}}  $};

\node [anchor = west,color= red] at (-8.2,-10.3) {~~~~~~$ RR_e(2\pi R_T) \approx (2ct_0)^3 O_B \approx l_0^3 ~ e^{a_{00} -1 \approx ~ a + 1}~~~~ a_{00}= i^{-i\pi} = e^{\pi^2/2}$};

\node [anchor = west,color= red] at (-5.2,-19.3) {~~~~~~~~~~$ \frac {g ~\approx~  c/annee}{g_0} = gt_0^2/l_0 \approx sin(2 \theta)\sqrt {a_{00}}  \approx \frac{2Z}{W} (S \approx \frac{u}{q}) \approx \pi^2 $};

\node [anchor = west,color= red] at (-14.2,-23.3) {$ \frac {ct_0}{coudee} \approx \frac {l_0}{r_{ADN}} \approx \frac {2}{\pi} \frac {2v_T t_0}{l_0} \approx \frac {p_{hol}^3}{4\pi} \approx 150^4 \approx p^{8/3}  ~~~~~~~~~ v_T = \frac {2\pi R_T}{Jour} ~~~~~~~~~ r_{ADN} \approx 2 nm  $};


\node [anchor = west,color= blue] at (-21.2,-19.3) {~~~~~~$f(2\pi) \approx (\frac {3R}{2\lambdabar_e})^{\frac {1}{30}} \approx \frac {t_K}{t_{ST} \approx 495 s}  $};
...
\node [anchor = west,color= blue] at (-28.2,-27.6) {~~~~~~$f(2) = \sqrt{f(6)}  \approx \frac {\sqrt 2}{g_1} \approx \frac {\sqrt 7}{g_2}  \approx \frac {\sqrt \Gamma}{g_3}  \approx \frac {3}{g_Z}  \approx 2\pi g_0 \approx 9\mu/a \approx 4an_t/H^0 $};
\node [anchor = west,color= red] at (0,-29.6) {~~~~~~$\pi/e \approx \Phi^q \approx 4\pi q^2 \approx (4\pi /3)(g_0 \approx g_1 \sqrt g_2)^3 $};


%$ \approx l_P \sqrt{O_M}$ \approx \sqrt{\lambdabar_p l_0}$ (mètre)};w_5 = 5(1-e^{-w_5}) 
...
%\node [anchor = west,color=black] at (-3.0,-3.0){$~$};
...
%\node [anchor = east,color=black!40!green] at (-20,-1.) {$f(31/2) \approx 2^{127/6}}; 
%node [anchor = east,color=black!40!green] at (0.,0.0) {$ m_e~ f(16) /\Phi_0^2 ~~ \approx $ masse du}};
%\node [anchor = west,color=black!40!green] at (21,0.0) {\Large{$\times~\Phi_0^2$}};
...
\node [cross,anchor = west,color=blue] at (3.45,3.15){$ T_{C} \approx 2.73 ~Kelvin \approx T_{H_2}T_{O_2}/T_{H_2O} \approx T_{mam}/j $};
\node [cross,anchor = west,color=blue] at (16.0,1.8){$ hc/k_BT_{mam} \approx \sqrt{Rl_P} $};

\node [cross,anchor = east,color=black!40!green] at (3.15,6.15){$RAM_{137}\approx a \sqrt{P}/H ~ (8 ppm)\approx f(18) \times u^2$};
\node [cross,anchor = west,color=red] at (6.15,6.15){$ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~4\pi (\frac {R_e}{\lambda_{Wien}})^2 \approx e^a$};
\node [cross,anchor = west,color=blue] at (6.15,6.15){$ {\lambda_{Wien}} \approx \lambda_{e} ( \frac {P}{a^2H^2}\approx \frac {3}{\pi} f_{\pi}(17)\approx l_P f_{\pi}(24) ) $};
%\node [cross,anchor = east,color=red] at (3.15,3.15){$RAMANUJAN_{137}\approx a \sqrtP/H $};~~

\node [anchor = south] at (12.25,-33.0) {$22$};
\node [anchor = south] at (12.25,-36.0) {$5$};

%\node [anchor = west,color=black!40!black] at (8.0,8.5){$f(20) = e^{2^5}  \approx (5/3)^{2^6}/2~~; $};
\node [anchor = east,color=blue] at (30.0,8.25){$f(20) = e^{2^5}  \approx (5/3)^{2^6}/2 ~ \approx \frac {\sqrt{m_Pm_F}}{\pi_a sin\theta} \approx \frac {sin^2 \theta~ l_0 }{r_H} \frac {l_0 }{sin \theta  \sqrt {\lambdabar_e \lambdabar_{\mu} }  }$};

\node [anchor = east,color=black] at (12.25,12.250){BOSON UNIFIANT $X_{GUT}$};
\node [anchor = west,color=blue] at (12.25,12.0){NEURON $t_N \approx  \frac {8\pi e t_0}{3570}~ (t_0 =$ 1 seconde) };
\node [anchor = east,color=black] at (14.,14.){$\pi^2 f(23) \approx \frac {l_P}{\lambdabar_e} = \frac {m_P}{m_e}\approx$};
\node [anchor = east,color=blue] at (30.,14.){  MASSE OVOCYTE $/m_e $};
...
%\node [anchor = west,color=red] at (18.,14.5){$l_K/ \lambdabar_e \approx u_n^{210}$ };
...
\node [anchor = east,color=red] at (-12,18.850){$2^{2^7-1}-1 $ : NOMBRE D'EULER};
\node [anchor = south,color=red] at (18.25,-33.0) {$\Gamma$};
\node [anchor = east,color=black!40!green] at (20.,20.0){$GRAVITONDE$};
\node [anchor = west,color=blue] at (20.,20.0){$R_{Hubble} \approx R_T \frac{P}{a_s (a+1)}$};
%\node [anchor = west,color=blue] at (20.,20.0){$R_{HUB}\approx R_T \frac{P}{a_s (a+1)}}$};
%\node [anchor = east ,color=red] at (21.3,20.5){$\frac {26}{6} approx~ $ NOMBRE PREMIER DE LUCAS $(2^{2^{2^{2^2-1}-1}-1}-1)\times 12$~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~};
\node [anchor = south] at (16.25,-33.0) {\textbf{24}};
\node [anchor = east ,color=blue] at (21,20.2){~$\frac {f(26)}{6} \approx 2^{2^7} \approx (f(-5))^{211} \approx (\frac{2}{\Phi})^{2\times 209} \approx \frac{R}{\lambdabar_e} \approx (\frac{2}{u})^{210} \approx u^{3570q^2} \approx(\frac{6}{\pi})^a \approx(\frac{g_3}{g_2})^{a_{00}-1}    $~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~};
%\node [anchor = east,color=black!40!green] at (28.25,28.250){ Super Période (Cosmon) $\hbar^3/G^2m^5 \approx \hbar^4/G_F^{5/2}\rho_c^{3/2}$ };
\node [anchor = east,color=red] at (28.25,27.750){$(6/5)^{2^{10}} \approx 2^{256}pn_t\beta^4/18^2~ (0.1 ppm) \approx Ct_0/\sqrt{a_0}\lambdabar_e  \approx (H/24) N_{Ed} \approx f(2e^e)/a_s^2 \approx (H/2\pi) (f(30) \approx t_{sp}/t_e)~~$ };
%\node [anchor = east,color=red] at (23.95,26.250){$f(29) \approx$ (NOMBRE DE RAMANUJAN)$^4 e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-Z/H^0}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm)$ };
\node [anchor = east,color=black!40!green] at (27.95,26.250){$O_M \approx (a^{1/3}/2)f(1/\delta) \approx (d_e/a_s)(\delta+1)^{3570}  \Rightarrow DOUBLE~ a   \approx 137.0359999099=$ mesuré (0.15 ppb)~~~~~~~~};
%\node [anchor = west,color=black!40!green] at (27.95,26.250){$\delta = a-137$};
\node [anchor = east,color=black!40!green] at (25.95,24.50){$ f_3(1/\delta) \approx \sqrt a sin \theta (H/p)e^a \Rightarrow a = 137+\delta \approx 137.0359999099$: idem (0.03 ppb)~~~~~~~~};
\node [anchor = east,color=black] at (29.25,29.250){$f(\sqrt{H/2}) \approx M/m_e = Rc^2/Gm_e = R\lambdabar_e/2l_P^2 = \lambdabar_e/TOPON$ (longueur d'onde de l'Univers) $\approx f(30) \times 153^2$};
\node [anchor = east,color=black] at (-5.25,10.250){\LARGE{MICRO-PHYSIQUE}};
\node [anchor = west,color=red] at (-5.25,10.250){$(ct_N/\lambdabar_e)^2 \approx R_e/2\lambdabar_e \approx D_{Eg}/l_P ~;~D_{Eg} \approx \sqrt {2R_T l_0} = 3570~ l_0  \Rightarrow  l_0 = 1 metre   $};
%\node [anchor = east,color=black] at (-8.25,8.250){$Y=\lambdabar_e/\lambdabar_{MICRO}$};
...
\node [anchor = east,color=blue] at (22.25,-16){\LARGE{MACRO-PHYSIQUE}};
%\node [anchor = east,color=black!40!green] at (22.25,-20.250){\LARGE{MACRO-PHYSIQUE}};
%\node [anchor = east,color=blue] at (14.25,-22.250){$Y = \l_{MACRO}/\lambdabar_{e}$};
\node [anchor = south] at (20.25,-33.0) {$26$};
\node [anchor = south] at (20.25,-36.0) {$6$};
%\node [anchor = south] at (30.25,-36.0) {$12$};
\node [anchor = south] at (28.25,-33.0) {$30$};
\node [anchor = south] at (28.25,-36.0) {$7$};
\node [anchor = south] at (31.25,-36.0) {$e^2$};
\node [anchor = south] at (29.25,-31.0) {$ \sqrt{H/2}$};
...
%\node [anchor = north] at (-24.55, 31.4) {$2e ~~~~ };{
...
\node [anchor = north] at (-33,-1.5) {$495^2 = H^0~~~~~~~~$};
...
\end{tikzpicture}
{\color{black}
%l'extrapolation de la double corrélation des grands nombres d'Eddington, qui s'identifie avec= t_0 = G_F^{5/4}/\hbar^2 G^{3/2}

\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les rapports de longueur $Y /\lambdabar_e$ entourent la loi $e^{2^{d/4}}$, basée sur 8 relations holographiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série spectroscopique, $d = 4k + 2$, de $k = 0$ à $k = 7$, séquence d'octonions de Bott. \textbf{L'Univers observable est permanent, boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Faible (k=3), Fort (Gluons k=1)}; Pour k = 6, d = 26 donne 6 fois le rayon de Hubble $R$, double du Nombre Premier de Lucas $2^{127}-1$, et le Topon correspond à $M/m_e \approx f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e ~= p \approx 1836.152673 $. D'où $R = 2a_G \lambdabar_e  \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse liée à la Super-Période $ t_{sp} =\hbar^4/G_F^{5/2}\rho_c^{3/2} = \hbar^3/G^2m^5 \approx 5.4829 \times 10^{57} ~s$, avec la constante de Fermi $G_F \approx 1.435850991 \times 10^{-62} kg~ m^5~ s^{-2}$ et la densité critique $\rho_c = 3c^2/8\pi R^2 = M/(4\pi R^3/3)$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = R \times (u = Hp/a^3 \approx e^{2/e^2} ) = 2\hbar^2/Gm_N^3$, entrant $(\frac {2}{\Phi} )^{2 \times 209} \approx 2 \sqrt{10} R_e/3\pi \approx d_e^2 WR/Z $~ (7ppm). $u_n = n_t^2/a^3 $. $N_{Ed} = 136 \times 2^{256} \approx (3/10)M/m_n$ implique $n_t = m_n/m_e \approx 1838.683659 \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). $f(18)/ \pi \approx$ le Nombre Premier d'Euler et le rapport thermique, relié aux points triples de l'Hydrogène, de l'Oxygène et de l'Eau. \textbf{Le cardinal du Monstre $O_M$ définit doublement la constante électrique $a \approx 137.035999099 $, conforme à la mesure (0.15 ppb), avec $a_s = a_w/2\pi (Hp)^{3/2}$, confirmé à 0.03 ppb avec $\sqrt a sin\theta = 2H^0 \sqrt{2+ln \pi} /W$}, et vérifie $H^{16} \approx O_M \times \phi_0^8$, avec $\phi_0^8 \approx 48$. La période de Kotov vérifie $t_{sp}/t_K \approx O_M/\sqrt2 \approx e^a/F$ et $ct_K \approx d_{Soleil-Uranus}$. Le rayon holographique du proton $p_{hol}^2 = 4a^3/3$ apparait dans $f(29) \approx N_{Ram}^4 = e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-Z/H^0}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm)$. La dimension 12 confirme la symétrie d'Eddington Proton-Tau, hors Modèle Standard. Pour n = 137 La fonction de partition de Ramanujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ confirme $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (10 ppm) et implique $p^{p^2}\approx (a^2)^{a^3}$ et $ f(a^{2/3})\approx (4\pi/3) \sqrt{f(26)f(27)}$. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes de la famille du Montre est lié à $a^a$ et au volume du Cosmos. $f(14) = f(10)^2  \Rightarrow g_Z = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 \Rightarrow$, boson de Higgs = $495^2 ~ (10 ppm)$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/Gm_e;~ P=m_P/m_e;~H = m_H/m_e \approx 1837.152646~;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652;~ j= 8\pi^2/ln2;~ \Phi = (1+\sqrt5)/2$ (Nombre d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nombre d'Argent). \textbf{La masse du Bicodon d'ADN est centrale}, liée au Monstre $ O_M \approx (\Phi_0 H^2)^8$. En particulier $\sqrt{Rl_P}\approx hc/k_BT(40^0C)$. Le Neuron $ t_N = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ est lié à $\sqrt {a_0} = a_w/WZ$ et la distance égyptienne liée au mètre $l_0$ et rayon terrestre par $D_E = \sqrt{2l_0R_T}$~= 3570 mètre et vérifie $Ct_N / \lambdabar_e \approx 5^2 f(30)$. Couplages de jauge : $g_1 = Zsin\theta/H^0, g_2 = W/H^0, g_3/g_2 = g_0/g_1 \approx 6/ \pi \approx \Phi^{4/3}, 1/g_0 = 1+g_1^2+g_2^2 = 2a^3/pp_G$, $\pi_a^2 = a^2-137^2 \approx aW/\sqrt {150} Z ; w_5 = \frac {hc/k_BT}\lambda_{Wien} = 5(1-e^{-5}) $ Déterminant d'Eddington :$\Delta_{Ed} = \sqrt{136^2-40}$. Mixage ~$S \approx 1/0.23121 ~(2 \times 10^{-4}).  $} }}

%Le Neuron $ t_0 = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ vérifie $Ct_0/\lamdabar_e \approx 5^2 f(30)$, confirmant la supercélérité $C$.} }

%Le Neuron %t_0 = G_F^{5/4}/\hbar^2 G^{3/2} \approx 19.9 ms$ est relié à l'octave 10 de la tierce mineure 6/5} }.
%Le Neuron t_0 = G_F^{5/4}/\hbar^2 G^{3/2} \approx 19.9 ms est fondamental }$ }}
%La masse du Bicodon d'ADN $m_{bc}\approx m_H \sqrt{m_n/m_p}$ apparaît comme centrale, à travers sa longueur d'onde $\hbar/$m_{bc}c$. 
...
%\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les rapports de longueur $Y /\lambdabar_e$ entourent la loi $e^{2^{d/4}}$, basée sur 8 relations holographiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série spectroscopique $d = 4k + 2$, de $k = 0$ à $k = 7$: \textbf{Le Tableau Périodique est étendu à une séquence d'octonions de Bott. L'Univers observable est permanent, boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Faible (k=3), Fort (Gluons k=1)}; Pour k = 6, f(26) donne 6 fois le rayon de Hubble $R$, double du Nombre Premier de Lucas $2^{127}-1$, et le Topon correspond à $M/m_e \approx f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e ~= p \approx 1836.152673 $. D'où $R = 2a_G \lambdabar_e  \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse liée à la Super-Période $ t_{sp} =\hbar^4/G_F^{5/2}\rho_c^{3/2} = \hbar^3/G^2m^5 \approx 5.4829 \times 10^{57} ~s$, avec la constante de Fermi $G_F \approx 1.435850991 \times 10^{-62} kg~ m^5~ s^{-2}$ et la densité critique $\rho_c = 3c^2/8\pi R^2 = M/(4\pi R^3/3)$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = R \times (u = Hp/a^3 \approx e^{2/e^2} ) = 2\hbar^2/Gm_N^3$, entrant $(\frac {2}{\Phi} )^{2 \times 209} \approx 2 \sqrt{10} R_e/3\pi \approx d_e^2 WR/Z $~ (7ppm). $u_n = n_t^2/a^3 $. $N_{Ed} = 136 \times 2^{256} \approx (3/10)M/m_n$ implique $n_t = m_n/m_e \approx 1838.683659 \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). $f(18)/ \pi \approx$ le Nombre Premier d'Euler et le rapport thermique, relié aux points triples de l'Hydrogène, de l'Oxygène et de l'Eau. \textbf{Le cardinal du Monstre $O_M$ définit doublement la constante électrique $a \approx 137.035999099 $, conforme à la mesure (0.15 ppb), avec $a_s = a_w/2\pi (Hp)^{3/2}$, et vérifie $H^{16} \approx O_M \times \Phi_0^8$, avec $\Phi_0^8 \approx 48$. La période de Kotov vérifie $t_{sp}/t_K \approx O_M/\sqrt2 \approx e^a/F$ et $ct_K \approx d_{Soleil-Uranus}$. Le rayon holographique du proton $p_{hol}^2 = 4a^3/3$ apparait dans $f(29) \approx N_{Ram}^4 = e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-Z/H^0}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm)$. La dimension 12 confirme la symétrie d'Eddington Proton-Tau, hors Modèle Standard. Pour n = 137 La fonction de partition de Ramanujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ confirme $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (10 ppm) et implique $p^{p^2}\approx (a^2)^{a^3}$ et $ f(a^{2/3})\approx (4\pi/3) \sqrt{f(26)f(27)}$. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes de la famille du Montre est lié à $a^a$ et au volume du Cosmos. $f(14) = f(10)^2  \Rightarrow g_Z = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 \Rightarrow$, boson de Higgs = $495^2 ~ (10 ppm)$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/Gm_e;~ P=m_P/m_e;~H = m_H/m_e \approx 1837.152646~;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652;~ j= 8\pi^2/ln2;~ \Phi = (1+\sqrt5)/2$ (Nombre d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nombre d'Argent). \textbf{La masse du Bicodon d'ADN est centrale}, liée au Monstre $ O_M \approx (\Phi_0 H^2)^8$. En particulier $\sqrt{Rl_P}\approx hc/k_BT(40^0C)$. Le Neuron $ t_N = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ est lié à $\sqrt {a_0} = a_w/WZ$ et la distance égyptienne liée au mètre $l_0$ et rayon terrestre par $D_E = \sqrt{2l_0R_T}$~= 3570 mètre et vérifie $Ct_N / \lambdabar_e \approx 5^2 f(30)$. Couplages de jauge : $g_1 = Zsin\theta/H^0, g_2 = W/H^0, g_3/g_2 = g_0/g_1 \approx 6/ \pi \approx \Phi^{4/3}, 1/g_0 = 1+g_1^2+g_2^2 = 2a^3/pp_G$, $\pi_a^2 = a^2-137^2 \approx aW/\sqrt {150} Z ; w_5 = \frac {hc/k_BT}\lambda_{Wien} = 5(1-e^{-5}) $ Déterminant d'Eddington :$\Delta_{Ed} = \sqrt{136^2-40}$. Mixage ~$S \approx 1/0.23121 ~(2 \times 10^{-4}).  $} }}


%\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les rapports de longueur $Y /\lambdabar_e$ entourent la loi $e^{2^{d/4}}$, basée sur 8 relations holographiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série spectroscopique $d = 4k + 2$, de $k = 0$ à $k = 7$: \textbf{Le Tableau Périodique est étendu à une séquence d'octonions de Bott. L'Univers observable est permanent, boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Faible (k=3), Fort (Gluons k=1)}; Pour k = 6, d = 26 donne 6 fois le rayon de Hubble $R$, double du Nombre Premier de Lucas $2^{127}-1$, et le Topon correspond à $M/m_e \approx f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e ~= p \approx 1836.152673 $. D'où $R = 2a_G \lambdabar_e  \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse liée à la Super-Période $ t_{sp} =\hbar^4/G_F^{5/2}\rho_c^{3/2} = \hbar^3/G^2m^5 \approx 5.4829 \times 10^{57} ~s$, avec la constante de Fermi $G_F \approx 1.435850991 \times 10^{-62} kg~ m^5~ s^{-2}$ et la densité critique $\rho_c = 3c^2/8\pi R^2 = M/(4\pi R^3/3)$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = R \times (u = Hp/a^3 \approx e^{2/e^2} ) = 2\hbar^2/Gm_N^3$, entrant $(\frac {2}{\Phi} )^{2 \times 209} \approx 2 \sqrt{10} R_e/3\pi \approx d_e^2 WR/Z $~ (7ppm). Intervient aussi $u_n = n_t^2/a^3 $. Le nombre d'Eddington $N_{Ed} = 136 \times 2^{256} \approx (3/10)M/m_n$ implique $n_t = m_n/m_e \approx 1838.683659 \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). $f(18)/ \pi \approx$ le Nombre Premier d'Euler et le rapport thermique, relié aux points triples de l'Hydrogène, de l'Oxygène et de l'Eau. \textbf{Le cardinal du Monstre $O_M$ définit doublement la constante électrique $a \approx 137.035999099 $, conforme à la mesure (0.15 ppb), avec $a_s = a_w/2\pi (Hp)^{3/2}$, et vérifie $H^{16} \approx O_M \times \phi_0^8$, avec $\phi_0^8 \approx 48$. La période de Kotov vérifie $t_{sp}/t_K \approx O_M/\sqrt2 \approx e^a/F$ et $ct_K \approx d_{Soleil-Uranus}$. Le rayon holographique du proton $p_{hol}^2 = 4a^3/3$ apparait dans $f(29) \approx N_{Ram}^4 = e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-g_Z}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm)$. La dimension 12 confirme la symétrie d'Eddington Proton-Tau, hors Modèle Standard. Pour n = 137, la fonction de partition de Ramanujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ confirme $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (10 ppm) et implique $p^{p^2}\approx (a^2)^{a^3}$ et $ f(a^{2/3})\approx (4\pi/3) \sqrt{f(26)f(27)}$. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes de la famille du Montre est lié à $a^a$ et au volume du Cosmos. $f(14) = f(10)^2  \Rightarrow g_Z = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 \Rightarrow$, boson de Higgs = $495^2 ~ (10 ppm)$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/Gm_e;~ P=m_P/m_e;~H = m_H/m_e \approx 1837.152646~;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652;~ j= 8\pi^2/ln2;~ \Phi = (1+\sqrt5)/2$ (Nombre d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nombre d'Argent). \textbf{La masse du Bicodon d'ADN est centrale}, liée au Monstre $ O_M \approx (\Phi_0 H^2)^8$. En particulier $\sqrt{Rl_P}\approx hc/k_BT(40^0C)$. Le Neuron $ t_N = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ est lié à $\sqrt {a_0} = a_w/WZ$ et la distance égyptienne liée au mètre $l_0$ et rayon terrestre par $D_E = \sqrt{2l_0R_T}$~= 3570 mètre et vérifie $Ct_N / \lambdabar_e \approx 5^2 f(30)$. } }}

%Couplages de jauge : $g_1 = Zsin\theta/H^0, g_2 = W/H^0, g_3/g_2 = g_0/g_1 \approx 6/ \pi \approx \Phi^{4/3}, 1/g_0 = 1+g_1^2+g_2^2 = 2a^3/pp_G$, $\pi_a^2 = a^2-137^2 \approx aW/\sqrt {150} Z ; w_5 = \frac {hc/k_BT}\lambda_{Wien} = 5(1-e^{-5}) $ Déterminant d'Eddington :$\Delta_{Ed} = \sqrt{136^2-40}$. Mixage ~$S \approx 1/0.23121 ~(2 \times 10^{-4}).  $



%\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les rapports de longueur $Y /\lambdabar_e$ entourent la loi $e^{2^{d/4}}$, basée sur 8 relations holographiques 2D-1D imbriquées micro-macrophysiques, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série atomique $d = 4k + 2$, de $k = 0$ à $k = 7$: \textbf{le Tableau Périodique est étendu à une séquence d'octonions de Bott. L'Univers observable est permanent, boson de jauge terminal du Cosmos:  U (k = 7), GUT (k= 5), Faible (k=3), Fort (Gluons k=1)}; Pour k = 6, f(26) donne 6 fois le rayon de Hubble $R$, double du Nombre Premier de Lucas $2^{127}-1$, et le Topon correspond à $M/m_e \approx f(30) \times 153^2$ où $12\times 153 = 1836 \approx m_p/m_e ~= p \approx 1836.152673 $. D'où $R = 2a_G \lambdabar_e  \approx 2 \hbar^2/Gm_p^2m_e = 2\hbar^2/Gm^3 $, où $m$ est la masse liée à la Super-Période $ t_{sp} =\hbar^4/G_F^{5/2}\rho_c^{3/2} = \hbar^3/G^2m^5 \approx 5.4829 \times 10^{57} ~s$, avec la constante de Fermi $G_F \approx 1.435850991 \times 10^{-62} kg~ m^5~ s^{-2}$ et la densité critique $\rho_c = 3c^2/8\pi R^2 = M/(4\pi R^3/3)$. La masse de Nambu $m_N = a m_e$ définit le rayon holographique réduit du Cosmos $R_e = R \times (u = Hp/a^3 \approx e^{2/e^2} ) = 2\hbar^2/Gm_N^3$, entrant $(\frac {2}{\Phi} )^{2 \times 209} \approx 2 \sqrt{10} R_e/3\pi \approx d_e^2 WR/Z $~ (7ppm). $u_n = n_t^2/a^3 $. $N_{Ed} = 136 \times 2^{256} \approx (3/10)M/m_n$ implique $n_t = m_n/m_e \approx 1838.683659 \approx \pi \times (153^2 +(e/2)^2)/40$ (10 ppb). $f(18)/ \pi \approx$ le Nombre Premier d'Euler et le rapport thermique, relié aux points triples de l'Hydrogène, de l'Oxygène et de l'Eau. \textbf{Le cardinal du Monstre $O_M$ définit doublement la constante électrique $a \approx 137.035999099 $, conforme à la mesure (0.15 ppb), avec $a_s = a_w/2\pi (Hp)^{3/2}$. La période de Kotov vérifie $t_{sp}/t_K \approx O_M/\sqrt2 \approx e^a/F$ et $ct_K \approx d_{Soleil-Uranus}$. Le rayon holographique du proton $p_{hol}^2 = 4a^3/3$ apparait dans $f(29) \approx N_{Ram}^4 = e^{4\pi \sqrt{163}}/2p_{hol} ~(1 ppm) \approx e^{....-Z/H^0}a/8\beta (Hpn_t)^{1/3} ~ (3 ppm)$. La dimension 12 confirme la symétrie d'Eddington Proton-Tau, hors Modèle Standard. Pour n = 137, la fonction de partition de Ramanujan $e^{\pi \sqrt{2n/3}}/4n \sqrt3$ confirme $G \approx 6.67545 \times 10^{-11} kg^{-1}m^3 s^{-2}$ (10 ppm) et implique $p^{p^2}\approx (a^2)^{a^3}$. Le nombre de photons dans l'Univers est lié aux cardinaux du Monstre et du Bébé-Monstre par $n_{ph}\approx (137/90)^2O_MO_B$ (10 ppm). Le produit des 20 groupes de la famille du Montre est lié à $a^a$ et au volume du Cosmos. $f(14) = f(10)^2  \Rightarrow g_Z = Z/495^2 \approx \mu^{-\mu/\tau} \approx 1838.5\sqrt2/3570 \Rightarrow$, boson de Higgs $ H^0 = 495^2 ~ (10 ppm)$, lié à $\Gamma = \gamma a/\pi$, la constante d'Atiyah;~$\mu \approx 206.7682869$;~$\tau \approx 3477.441701$;~ $m_P^2 = \hbar c/Gm_e;~ P=m_P/m_e;~H = m_H/m_e \approx 1837.152646~;~ \sqrt a_0 = a_w/WZ;~ W = m_W/m_e \approx 157340.1093;~ Z = m_z/m_e \approx 178451.7529; ~cos\theta = W/Z; ~ F = \sqrt a_w \approx 573007.3652;~ j= 8\pi^2/ln2;~ \Phi = (1+\sqrt5)/2$ (Nombre d'Or) ;~ $\phi = 1 +\sqrt2 $ (Nombre d'Argent). \textbf{La masse du Bicodon d'ADN est centrale}, liée au Monstre $ O_M \approx 48 H^{16} \approx (\Phi_0 H^2)^8$. En particulier $\sqrt{Rl_P}\approx hc/k_BT(40^0C)$. Le Neuron $ t_N = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ vérifie $Ct_N / \lambdabar_e \approx 5^2 f(30)$ et est lié à la seconde par le même nombre canonique 3570 qui relie le mètre $l_0$ et le rayon terrestre $R_T$. Charge électrique $q= g_1 cos \theta + g_2 sin\theta \approx 2((2+ln\pi)/a)^{1/2}$. Couplages de jauge : $g_1 = Zsin\theta/H^0, g_2 = W/H^0, g_3/g_2 = g_0/g_1 \approx 6/ \pi \approx \Phi^{4/3}, 1/g_0 = 1+g_1^2+g_2^2 = 2a^3/pp_G$, $\pi_a^2 = a^2-137^2 \approx aW/\sqrt {150} Z ; w_5 = \frac {hc/k_BT}{\lambda_{Wien}} = 5(1-e^{-5}) $. Déterminant d'Eddington :~$\Delta_{Ed} = \sqrt{136^2-40}$. Mixage ~$S \approx 1/0.23121 ~(2 \times 10^{-4}).  $} }}

%Le Neuron $ t_0 = G_F^{5/4}/\hbar^2 G^{3/4} \approx 19.14 ~ms $ vérifie $Ct_0/\lamdabar_e \approx 5^2 f(30)$, confirmant la supercélérité $C$.} }  confirmé à 0.03 ppb avec $\sqrt a sin\theta = 2H^0 \sqrt{2+ln \pi} /W$}
...
%\approx (4\pi 495^2 W/a_w)^2/3 \approx (9/10)^3 p/\sqrt{n_tH}\approx \mu^{-\mu/\tau}/\beta $ dans $pF\approx (gH)^3/(\mu/a)^2$, 
%}
