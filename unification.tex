...
Combined R23W: $v_d = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & $v_{12}$    & $ \underline {1831} $ & 30D to 12D \\   
 R23W for $d$ = 8:  Eddington's formula\cite{Eddington}: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & $v_{8}= 495 + 496$ \\      
  ...
  A-dimensional Electric charge $q \approx 2 a^{1/4} (8/5)^8 \tau/pH \approx (6^2/\beta d_e)^{-1/3}$  & $q$ & 0.30297311 & $6 \approx 2^{137/53} $\\
...
Opt. Strong (inverse) coupling constant $a_w/2\pi(pH)^{3/2} \ approx (\pi H^{3/2}/\tau)^{1/2}$ & $a_s$   & $8.434502906$ & \cite{Sanchez2} $a_s \approx g_1^{-2}(p_t/H)^4$ \\
...
\label{CosmosTable}
 ...
 $ l_P ~(2/\sqrt5)(80)^{64}$ & 9.07510 & Implying the musical property (16 ppb): $((3/2)^4/5)^{256} \approx 24 (a/137)^{32}(p_t/p_W)^{1/2}$ \\
...
$ 2(a/137\beta)^2 ~l_{nl}^4 ~\lambdabar_{bc}^{(0)}/R_e \lambdabar_el_P^2 $ & 9.07580 & Holic Principle, with the reduced wavelength of the DNA bi-codon \\
 ...
 $ l_P ~(g_1g_2g_3)^{-210}~i^{i\pi} $ & 9.06 & Holic Principle with base $(g_1g_2g_3)^{-1}$, confirming the pertinence of $i^{i\pi}= e^{\pi^2/2}$   \\
 ...
 $ l_P~ e^{27aq/4} ((2W+Z)/3W)^2(n_t/p_t)^2$& 9.076 & Confirms the charge $q$   \\
  ...
  $  \lambdabar_e ~  ((3/4\pi)(R_1/\lambdabar_e)^7)^{1/3}   $ & 9.078 & From the de photons nomber, using the mono-electron radius $R_1$ \cite{Sanchez2}  \\
  ...
  $ l_P ~b_3^{210}~a/\pi $ & 9.06 & Holic Principle with base $b_3 = 2 + \sqrt 3$, the Lucas primality test generator \cite{Sanchez5}  \\
   ...
   $ l_P ~e^{(n_t/a)^2/g_2}~(3/\pi)^{1/2} $ & 9.06 & Natural base $e$, Confirms $g_2$  \\
  $ \lambdabar_e ~e^{u_{30}} $ & 9.09 & Natural base $e$, with $u_{30} = (60 \times 61)/2 + 1$ \\
