...
{\color{red} \section{Plan}

The Introduction (Section 2) presents the research background, and the problems of the former researches, which are solved in this article.


It is recalled the Topological Axis and the main physical parameters and (Table 1, 2,3), including the values specified by the Optimal Computation Principle, which connect to 0.1 ppm with the Permanent Holographic Osillation Bang Cosmology (PHOB). A series of formula, based on several theories, confirm both the \textit{invariant} Hubble radius and the Cosmos radius.
 
The Section 3 recalls the Holic Principle. In particular, it shows how the mass concept introduces a symmetry between gravitation and quantum physics in the Keplerian Diophantine equation. This connects the Bohr’s orbits with the PHOB model, showing up the mean DNA bi-codon mass which connects to 0.1 ppm with the Topological Axis. The local c-observable base 2 Universe appears as a gauge boson in the base 3 Tachyonic Cosmos with massive photon and graviton. This induces a scalar boson mass tied to the geometric extension of the Weinberg triangle, correlating the three gauge couplings U(1)-SU(2)-SU(3) with the gravity constant $G$.
...
The Section 5 studies t;he multi-dimensional cristallography, with emphasis to its relation with the $23^{th}$ celular automaton Wolfram series. In liaison with the string dimension reduction problem, this leads to a characterisation of the main integer parameters 137 and 1836. The latter is connected with the maximal Euler suitable number, tied to the generalized Riemann hypothesis. The base 2 Wieferich last prime permits to define the fine-structure constant within its 0.15 ppb indetermination. Considering the connection between crystallography and the Periodic Table, the running number of the string dimension series of the Topological Axis is identified with the orbital quantum number, associating the spin 1/2 and the string dimension 2.
...
\section{Introduction: the Topological Axis and the \textit{invariant} Hubble radius}\label{AxeTopologique}
...
Moreover, the standard model of particles in unable to explan the origin of about 20 "free parameters". This article shows they are connected with the Hubble radius.
