...
\node [anchor = east,color=red] at (16,17.250){Synth. triconcept. : $\hbar,G, m_{3570}^3 \rightarrow 2l_K \rightarrow masse_{photon} \approx masse_{graviton} \times (a_w \approx \frac {f(26)} {f(24)}$};

%\node [anchor = east,color=red] at (17.25,17.850){$f(25)\approx 2e^{24\pi}\approx O_B~ (W/F) \approx \sqrt {O_M}~ (8W \approx 7Z \approx 5aH) $};

\node [anchor = east,color=red] at (13.25,14.5){Synthèse triconceptuelle : $\hbar,G, m_e, m_p, m_H \rightarrow 52,255 Hz \rightarrow $ Lab (La = 442,9 Hz)  };
...
\node [anchor = north] at (-14, 34) {Synth. triconcept. : $\hbar,G, m_e, m_p, m_H \rightarrow R/2$};
