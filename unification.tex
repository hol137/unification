...
\node [anchor = east,color=blue] at (36.7,15.75){$ a^a \approx (e ^ {1/e})^{p_G (n_t/p)^  {1/2} } \approx (\sqrt {a_s} ^  {1/ \sqrt {a_s}})^{\sqrt {pH}} \approx ((sin \theta) ^  {-sin \theta})^{(a/\pi)^2} \approx (q ^  {-q})^{2(a sin^2 \theta)^2} \approx (\pi ^ {1/\pi})^{p_{hol}} \approx a_s^{2{p_{hol}/\sqrt a} \approx (h_0 ^  {1/h_0 })^{2(\frac {d_e(p_W+1)}{a})^3}  $};
