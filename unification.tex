...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Maximal known Euler Ideonal ("Suitable") Number  & $s_{65}$   & \underline {1848} & OEIS (A000926) \\
...
Using the main isotopes: $~^{1}_{1}H^{(0)} = H,~ ^{6}_{12}C = C^{(0)},~ ^{7}_{14}N = N^{(0)}, ~^{8}_{16}O = O^{(0)},~^{15}_{31}P= P^{(0)}$  \cite{Huang}, the masses of the 4 DNA nucleotides, by respect to the hydrogen mass $H$ are close to the Fermi mass ratio: $\sqrt {a_w/pH} \approx 311.9846$, very close to 312, the $53^{th}$ ideonal Euler number, while the Gyuanine nucleon number is 329 = 312 + (2+3+5+7), whose importance will appear in the next Section, in relation with the sum of primes $\sigma(p)$: 
...
(\tau -1)^{32}/f(32) \approx a^2/137
 ...
 \textit{So, the terminal dimension 32 of the Topological Axis is associated to $\tau$, the terminal lepton. The number three of particle families is therefore confirmed}. One notes the direct correlation implying \textit{the product} of the nucleotide mass ratios: $4d_e G_u^{(0)} C_y^{(0)} \approx 4d_e A_d^{(0)} T_h^{(0)} \approx (H/3)^2 \approx H^{(0)}/g_0 = H^{(0)} + Z^2/H^{(0)}$, confirming the pertinence of the hypothesis Eq.(9.1). This induces the symmetrical relation implying $s_{65} = 1848$, the last known Euler number:
...
\textit {The central importance of the scalar BEH boson is thus confirmed}. This shows also a symmetry between 1836 and 1848, which are the $34^{th}$ and $35^{th}$ areas of integer-sided triangles whose area equals 6 times their perimeter (A332879 in OEIS).
