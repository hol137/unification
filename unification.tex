...
\node [anchor = west,color=blue] at (-12.,-12.) {$3f(10) \approx 2\pi r_H$ (Rayon de l'atome d'Hydrogène) $\approx (2/u)^{16} \approx (3570)^{16}/O_M$ };
...
\label{tab:5:table5}
  \hskip-2.0cm\begin{tabular}{lllll}
    ...
    $m_e $ & $\sqrt {m_pm_H}$   & $R/2$: demi-rayon de l'Univers observable &exact& $R/\lambdabar_e \approx g(6) \approx 2^{2^7}\approx (2/u)^{210}$   \\
    $m_{bc}^{(0)}R_e/R $ & $\sqrt{ m_{ph}m_{gr}}$   & $R_C$: rayon du Cosmos = $RC/c = (R/2)m_N^3/m_{bc}m_{ph}m_{gr}$ & $4.7 \times 10^{-4} $& $ R_C/\lambdabar_e \approx e^{e^{2e}} \approx 6^{2^7}\approx (2/u)^{64a_s}  $\\
