...
\label{tab:2:table2}
  \hskip-2.0cm\begin{tabular}{llll}
    ...
    Euler-Napier constant and opt. nb. $e_{n+1}=e^{n}$;$e_4^{1/aa'}\approx (9/2)^{a'} \approx P^4 sin^2 \theta $~ ; $a' = a-1 $  & $e$    & 2.718281828459042 & $e_2/e_3^{1/4} \approx e^{-e^{e^{-e}}} \approx g_1$  \\    
    Archimedes constant $\pi$ and $\pi_a \approx 6/(1/g_1 - 1)\approx 3/(1-g_1g_2g_3/6)\approx 3 + (q/1 + 1/\Delta_{Ed})^2/g_2$ & $\pi$    & 3.14159265358979 & \pi_a = (a^2-137^2)^{1/2}\\    
    ...
    Golden ratio and Photon Length $\phi = \lambda/l_{ph} = (16\pi \xi(3))^{1/3}$ & $\Phi\approx \phi g_2^2$  & 1.61803398874990 & $\xi(3) \approx 1.20205691$ \\ 
    ...
    Wyler $ p_W = 6\pi^5 \approx (ap_t/137)^2/H \approx \sqrt p_t (s_{65}\sqrt {p_{00}}/p_{Ed})^3 /H\approx (Hp_{00}^7)^{1/2}/p_t^3$ & $p_W$   & \underline {1836.118109}  & \cite{Wyler} $P \approx a_w \sqrt a n_t^3 (p_W/p_t)^5$ \\ 
...
Combined R23W: ${\color{black}u'_d} = u_{2D = 2(d+3)} = 1 + 2(d+3) + 8 (d+3)^2$ for d = 12 & ${\color{black}u'_{12}}$    & $ \underline {1831} $ & 30D to 12D \\   
 R23W for $d$ = 8:  Eddington's formula\cite{Eddington}: $u_{8}= 136 + 1$ & $u_{8}$   & $ \underline {137} $ & $v_{8}= 495 + 496$ \\      
  ...
  $u_{2d} = 1 + 2d(4d+1)$ Bisection of odd central polygonal numbers for \underline {d = 4}  & $a_{00} $    & \underline {137}  & OEIS (A188135) \\
    ...
    Opt. Weak-mixing ratio $W/Z \approx 495 q^2 \sqrt a/ \beta^{1/4} lnP$ & $cos\theta  $ & 0.88169557 & 0.1 ppm \\
...
A-dimens. El. charge $q \approx 3/\pi^2 \approx (3/10)\sqrt(6F)/p_{00} \approx (6^2/\beta d_e)^{-1/3}\approx (6\mu/137g_0\Gamma)^2$  & $q$ & 0.30297311 & $6 \approx 2^{137/53}$ (indian music) \\
   Opt. SU(2) coupling $W/H^{(0)}\approx g_1(p\beta/p_W)^{1/2} \tau/9\mu \approx (p\beta/n_t)^{1/2}\pi \tau W/Fa^{3/2} $  &  $g_2$  & $0.6421390$ &  $g_2 \approx y/ \Phi^2 \beta^4 \approx \beta^4\pi g_1/y$  \\ 
