...
\subsection{Connexion with the Weak Bosons and the WFV number $K_4 = 165$}
...
The EOIS presents the crystallographic series (A120963), without citing Hermann, the first discoverer. By contrast, there is a reference to the Weigel team (Pham Veyssere and Veyseyyre), but without presenting the specific series of positive symmetry numbers $K_n$ for spaces of \textit {even} number of dimensions $n = 2d$: 5,19,59,165,419,1001,... (Table 7). The Weigel-Phan-Veysseyre series shows up the following "Transdimensionnal Theorem": the number of positive operations is the same for dimensions 2n and 2n+1. Moreover, the number of positive operations in dimension $n$ is equal to the number of negative operations in dimension $n+1$. This implies that, for an odd dimension the number of positive and negative operations are equal. Thus, the WPV series may be considered as the generator of the Hermann series: 

\begin{equation}
  K_{n-1}+ K_n = N_{2n}   ~~~~~;~~~~ 2K_n = N_{2n+1}
\end{equation}
 
So, among these WPV numbers $K_n$, 165 is both the number of positive point operations in dimensions 8 and 9, and also the number of negative point operations in dimensions 9 and 10. \textit {So the common 4-space is associated to the spaces of dimensions 8, 9 and 10. This is obviously tied to the superstring compactification from dimension 10}.
...
\begin{equation}
 \left\{
    \begin{array}{ll}
    2\times 1836 = \sum_1^{12} N_d = 4\sum_1^5(K_n) + 10^3 + 4 ~~~~;~~~~ 2\times \sum_1^5(K_n) = 11^3 + 3  ~~~~\Rightarrow \\
    \\ 
...
Introducing $K_6 = 1001$ and $K_7 = 2257$, one observes:

\begin{equation}
(K_5 +K_6 + K_7)/2  =  1838.5
\end{equation}

where 1838.5 is very close to the neutron mass ratio.
...
\begin{equation}
 \left\{
    ...
    (K_5 + K_6)/4 = 6 K_3 + 1 ~~~~\Rightarrow~~~~  \\   
   \\ 
   ~~~~~~~~~~~~~~~~~~~~~~~~ (K_3 + K_4 + K_5 + K_6)/12 = 2 K_3 + K_2 = 137
    \end{array}
\right.
\end{equation}
 ...
 \left\{  
    \begin{array}{ll}
       ...
       \\
        ...
        \\
        e \approx (K_{5}/(K_{5}-2))^{210-1} ~~~~ \rightarrow ~~~~  K_{5}/(K_{5}-2)\approx F^5/Pa^3  ~~~~(ppb)
 \\    
  ~~~~~~~ 417 = K_{5}-2 = 3(137 + 2) \approx 2 \times 210 \sqrt (g_1 + g_2)   ~~~~ (0.3 ppm)
...
Among the WPV numbers, only the $4^{th}$(165) and the $6^{th}$ (1001) are non-trivial binomial numbers, encadring the $81^{th}$ prime number $K_{5} = 419$. It is conjectured that these are the only two such cases. Taking account ot the above paramount importance of $K_4$, relations involving between these two numbers are looked for. Indeed: 

\begin{equation}
 \left\{
    \begin{array}{ll}
    (K_8 + K_{12})/2 = N_{10} - 1 \approx p_G/\pi \\
    \\
    2 + 3 + 5+ 7 = 17 \approx g_1^3 K_{10}     ~~~~~~\rightarrow~~~~~~ K_{10} /(K_{10} -2) \approx F^5/Pa^3 ~~~ (ppb)
    \\
    2\times 3 \times 5\times 7 = 210 = (K_{10}+1) /2 \approx 2K_{12} sin^3\theta \approx g_1 g_3 K_{12}/2       ~~~\Rightarrow~~\\
    \\
