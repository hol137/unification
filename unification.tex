...
\node [anchor = east,color=red] at (32,7){$\LARGE{ \frac {\lambdabar_e^2}{a^3 l_1 l_P}  \approx 60 (3570 \approx \Phi^{17}-1)^{1/2} ~~~(9 ppm)}$};
...
\node [anchor = west,color=red] at (-30,15.0){\LARGE{NEURON} $t_N \approx  \frac {8\pi e t_1}{3570}~ (t_1 =$ 1 seconde) };
...
\caption[Graphics \ref{AxeCosmique}: AXE COSMIQUE]{\footnotesize{Les paramètres physiques $Y$, rapportés à l'électron, entourent la loi en double exponentielle $e^{2^{d/4}}$, basée sur 8 relations holiques (c’est-à-dire sans paramètres topologiques) 2D-1D imbriquées micro-macrophysiques $Y(d+4) = (Y(d))^2$, d'où le \textit{double} logarithme $y = ln(lnY)$ qui dépend de la série spectroscopique $d = 4k + 2$, de $k = 0$ à $k = 7$: \textbf{Le Tableau Périodique (\underline {s}=2, \underline {p}= 6, \underline {d}=  10, \underline {f}= 14)  est étendu à une séquence d'octonions de Bott}.
Pour k = 6, $d$ = 26, le nombre de dimensions de la théorie bosonique des cordes, et f(26) est proche de 6 fois le rapport du rayon de Hubble $R$ (mesuré par la récession galactique) sur la longueur d’onde de l’électron $\lambdabar_e$. Ce rapport est voisin de $2^{128}$, soit le double du nombre premier de Lucas $2^{127}-1$, le grand nombre le plus célèbre de l’histoire des mathématiques.
...
La masse critique $M = Rc^2/2G$, telle que la vitesse de libération de l’Univers est $c$, peut être interprétée comme une relation holographique d'Archimède (le périmètre d'un cercle de rayon unité est deux fois son aire) faisant intervenir la longueur d’onde $d$ de l’Univers, appelée Topon, qui brise le mur de Planck d’un facteur $10^{60}$, et est inférieure à la longueur d’onde de l’électron du facteur $f(30) \times 153^2$. Ce nombre biblique 153 est le 17ième triangulaire.
Comme $153 \times 12 = 1836$ est très voisin du rapport de masse proton-électron, le carré de la longueur d’onde du proton intervient, et on retrouve la formule donnée par la synthèse tridimensionnelle donnant le demi-rayon $R/2$ de l'Univers, à partir des constantes $\hbar, G, m$, où $m$ est la moyenne des masses électron-proton-neutron, formule trouvée en 3 minutes, et déposée sous pli cacheté en 1998 (voir Wikipedia, Analyse dimensionnelle, exemple en Cosmologie). Le terme terminal $d$ = 30 (k = 7) est donc d’importance physique centrale. Or les valeurs impaires k = 3 et k = 5 correspondent aux bosons de jauge faibles et de grande Unification. Il reste donc une place et une seule pour les bosons forts, pour k = 1 (Gluons). La densité cosmique est proche d'1 atome pae coudée cube. Le rapport mètre/coudée est $(\pi/6)$, directement lié à $R$. Le rayon du Cosmos, obtenu par la symétrisation holographique la plus simple, divisé par le nombre atomique de l'Univers $n_H = M/m_H$ est lié au rayon de la Terre, lequel définit le mètre égyptien comme la hauteur d'un observateur dont l'horizon est de rayon 3570 mètre. Ce nombre 17x210 est l'antécédent de la puissance 17 du Nombre d'or, et relié aux nombres premiers holiques 2,3,5,7 dont la somme est 17 et le produit 210. Ce nombre est aussi lié à la base 60 : $3570 = 60^2 -60/2$, alors que le nombre correspondant en base 12 est $12^2-12/6 = 138$, le successeur du 137, qui est la partie entière de la constante électrique $a$. La seconde est la demi-période d'un pendule de longueur 1 mètre.  
Le $sin \theta$ de la théorie électrofaible corrèle aussi directement avec le nombre d'or.
Pour la valeur centrale $d$ = 16, l’axe donne une valeur très proche et corrélée avec la masse moyenne du bi-codon d’ADN. Avec $G_F$, la constante de Fermi, $G$ et $\hbar$ donnent un temps, le TEMPON qui s’identifie avec le temps de réflexe visuel et correspond au troisième octave inférieur du La bémol.
